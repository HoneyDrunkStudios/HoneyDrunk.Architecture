...
The substrate change does not authorize a runner-local prompt fork. Both review passes consume the same canonical agent file per [ADR-0007](./ADR-0007-claude-agents-as-source-of-truth.md). Follow-on packet 12 clarifies that `.claude/agents/review.md` owns the established Grid Review PR-comment format (`Risk Level`, `Review Confidence`, `✅ Verdict`, and the full emoji section checklist), so the runner synthesis prompt mirrors that canonical contract instead of inventing its own PR-facing shape. [ADR-0044](./ADR-0044-grid-aware-cloud-code-review-and-ai-authored-pr-discipline.md) D1's "drift between execution surfaces is forbidden" principle is preserved verbatim and re-expressed: the surfaces now are Codex CLI under subscription auth and Claude Code CLI under subscription auth (per D8 below), not OpenClaw + Codex.
...
The PR body's `Work Item:` / `Out-of-band reason:` metadata (per the user's PR metadata convention, memory-pinned) is the source of truth for packet linking. The Action extracts it; the comment carries it forward; the worker re-reads it from the comment rather than the PR body (idempotent against later body edits during the review window).
...
- **Auth surfaces:**
  ...
  - **Open-source safety gate:** the runner treats PR-authored content as hostile input. A fresh clone is inert: no committed `host.psd1`, and the example host config leaves `Safety.Enabled = false`. Non-dry-run jobs require an operator-local opt-in (`Safety.Enabled` and `Safety.OperatorAcknowledgedUntrustedInputs`) plus an explicit repository allowlist. The scheduled runner code is installed from an operator-controlled copy outside any Git worktree or configured repository path, so a malicious PR that later lands in Architecture does not automatically become the code Task Scheduler executes. The review job rejects fork and private-head PRs by default, requires the queue comment marker for claim recovery, never checks out or executes the PR head, ignores arbitrary PR-body links, passes only GitHub diff/context and trusted base-branch packet metadata to child agents, scrubs common cloud/source-control/API-token environment variables before launching child agents, and runs Codex in an ephemeral read-only mode that ignores repo-local rules. Any later decision to review forks/private heads or run repository-hosted runner code is a conscious host-local policy change, not a behavior inherited by cloning the Architecture repo.
...
- **`.claude/agents/review.md`** — canonical source for the PR-facing Grid Review format. Packet 12 clarifies the established emoji-section output contract in the agent file so the runner does not carry a runner-only review format.
...
- **HoneyDrunk.Vault** — the webhook-signing secret used for ADR-0044's primary path is rotated out per [ADR-0006](./ADR-0006-secret-rotation-and-lifecycle.md) discipline at cutover. The reused review-agent App credentials are stored in the shared automation Vault `kv-hd-automation-dev` under `GitHub--AgentRunner--AppId`, `GitHub--AgentRunner--PrivateKey`, and `GitHub--AgentRunner--InstallationId`. The worker reads the App credentials at tick startup, exchanges them for an installation token via `POST /app/installations/{installation_id}/access_tokens`, and uses the resulting short-lived token for the tick. Rotation remains the App's native key-rotation flow per ADR-0006.
...
- The Grid-aware agent's two execution paths consume the same `.claude/agents/review.md` definition and output contract; drift between paths is forbidden.
...
- **Clarify `.claude/agents/review.md` output contract via packet 12** — the transport change itself does not fork the prompt, but the canonical agent file must own the established Grid Review comment format that the runner posts.
