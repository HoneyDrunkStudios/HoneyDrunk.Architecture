...
| **Studios / TypeScript + native** | A TypeScript / React / web-UI repo that **also bundles a native bridge/shell crate (Rust) in one workspace** — a dual Node + Cargo workspace shipping a UI plus a co-bundled native binary, not a published package. | HoneyDrunk.HoneyHub (the Agent Cockpit — React+Vite PWA + Tauri-class shell + Rust bridge in one workspace repo) | As `studios-typescript`, plus a second Cargo lane. Delta vs `studios-typescript`: dual **Node + Cargo** workspace; CI runs **both** an npm/pnpm lane **and** a `cargo build`/`cargo test`/`cargo clippy` lane via a **self-contained `pr.yml`** (it does not call the .NET `pr-core.yml`); the required `main` status check is the job's own name (**`pr / build`**), not `pr-core / core`. No `.slnx` / `Directory.Build.props` / `HoneyDrunk.Standards` / NuGet. Org-secret matrix row: **none required by default** (no npm publish, no Sonar lane unless one is later added). Per-class walkthrough: stub (see D7). |

The taxonomy is closed at **seven** classes for the Grid as of 2026-06-06 (six original classes per the 2026-05-25 baseline plus `studios-typescript-native`, added by the 2026-06-06 amendment below). Future class additions (mobile apps per ADR-0070 D3, Bicep-only infra repos, additional Tauri desktop clients) require a one-row amendment to this D2 plus a per-class walkthrough; not a new ADR.

The class is declared in the standup ADR's frontmatter as a new optional field — `node_class: core-dotnet-abstractions-runtime` (or one of the other six values: `core-dotnet-runtime-only`, `ops-deployable-dotnet`, `meta-docs`, `ai-seed`, `studios-typescript`, `studios-typescript-native`). When omitted, the default is `core-dotnet-abstractions-runtime`. Authoring tools surface the class for human and agent review.
...
15. **`.github/copilot-instructions.md`** — per-repo file pointing back to Architecture's copilot-instructions plus any repo-specific addenda.
...
**Studios / TypeScript + native additions** *(stub — added by the 2026-06-06 amendment; full per-class walkthrough is a D7 TODO):*

- aa. Everything in the `studios-typescript` steps (x–z) applies to the TS/web-UI surface, **except** the CI workflow: this class ships a **self-contained `pr.yml`** that does **not** call the .NET `pr-core.yml`. The `pr.yml` runs an npm/pnpm lane (`npm ci && npm run build && npm test`, or the pnpm equivalent) **and** a Cargo lane (`cargo build && cargo test && cargo clippy`), both via direct CLI invocation (Invariant 38). The single required `main` status check is the job's own name — **`pr / build`**, not `pr-core / core`.
- bb. A **dual Node + Cargo workspace** (a Node/pnpm workspace for the UI/shell-frontend/shared-types packages plus a Cargo workspace for the native bridge/shell crate), in one repo. No `.slnx`, no `Directory.Build.props`, no `HoneyDrunk.Standards`, no NuGet.
- cc. **Org-secret matrix: none required by default.** No `SONAR_TOKEN` (no .NET `pr-core.yml` Sonar job runs unless a Sonar lane is later added to `pr.yml`), no `NPM_TOKEN`/`NUGET_API_KEY` (no package publish at v1 — the UI is a static build and the native crate is a co-bundled binary). Bind secrets only if/when a lane that consumes one is added.
- dd. **TODO (D7 walkthrough):** `infrastructure/walkthroughs/node-standup-studios-typescript-native.md` — the full per-class operational step-by-step. Until it lands, this stub plus the `studios-typescript` walkthrough are the reference.
...
- **Retired ADR-0044 review-pipeline emission row:** the former OpenClaw webhook-signing secret was removed under ADR-0088. Nodes no longer bind a review-result upstream-emission secret.
...
## Amendment (2026-06-06) — Add the `studios-typescript-native` Node class (D2 one-row addition + stub class-specific steps)

**Status of the amendment:** a **one-row D2 taxonomy addition** plus a stub per-class entry under D5, exactly the lightweight change this ADR pre-authorizes ("Future class additions ... require a one-row amendment to this D2 plus a per-class walkthrough; not a new ADR"). The original six classes and all D1/D3–D8 decisions are **unchanged**. This amendment does not edit the D-prose above except to insert the seventh row in the D2 table, extend the `node_class:` value list, and append the `studios-typescript-native` stub steps (aa–dd) under D5; the deltas are recorded here as the amendment of record.

**What changes.** The Grid's first mixed TypeScript-UI + native-bridge repo — `HoneyDrunk.HoneyHub` (the Agent Cockpit: a React+Vite PWA + a Tauri-class shell + a Rust bridge crate in one workspace repo, per ADR-0091) — does not fit cleanly into `studios-typescript`, because that class assumes a pure Node/TS toolchain with a single npm CI lane. The new class **`studios-typescript-native`** captures the dual footprint as a first-class taxonomy row rather than forcing a TS class with a bolted-on lane.

**Delta vs `studios-typescript`:**

- a **dual Node + Cargo workspace** in one repo (a Node/pnpm workspace for the UI/shell-frontend/shared-types packages; a Cargo workspace for the native bridge/shell crate);
- CI is a **self-contained `pr.yml`** that does **not** call the .NET-shaped `pr-core.yml` (which does not have, and at the time of this amendment HoneyDrunk.Actions does not ship, a TypeScript/`pr-typescript.yml` reusable workflow). `pr.yml` runs both an **npm/pnpm lane** (`npm ci && npm run build && npm test` or pnpm equiv) and a **Cargo lane** (`cargo build && cargo test && cargo clippy`), each via direct CLI invocation per Invariant 38, until/unless a shared `pr-typescript.yml` is built (per the ADR-0082 D5y "invoke CLIs directly until that workflow is built" path);
- the required `main` branch-protection status check is the `pr.yml` job's own name — **`pr / build`**, not `pr-core / core`;
- **no** `.slnx` / `Directory.Build.props` / `HoneyDrunk.Standards` / NuGet;
- **org-secret matrix row: none required by default** — no npm publish (the UI is a static build, the native crate is a co-bundled binary), no Sonar lane unless one is later added to `pr.yml`. `SONAR_TOKEN` is therefore **not** a standup prerequisite for this class.

**No new invariant.** This amendment adds a taxonomy row and class-specific steps; it does **not** add a new `constitution/invariants.md` entry. It **does amend invariant 102 clauses 8 and 9** to carry the class-specific CI carve-out: a non-.NET Node class with an ADR-0082 D2 CI contract may use a self-contained `pr.yml` + its own required check (e.g. `pr / build`) in place of `pr-core.yml` / `pr-core / core`. This aligns 102's text with invariant 31, which already scopes `pr-core.yml` to .NET repos. Invariant 102's ten registration items otherwise still bind every Node of this class.

**Follow-up.** A full per-class walkthrough `infrastructure/walkthroughs/node-standup-studios-typescript-native.md` (the D7 deliverable for the new class) is a TODO; until it lands, the D5 stub (aa–dd) plus the existing `node-standup-studios-typescript.md` walkthrough are the reference. `constitution/node-standup.md` is updated in lockstep with this amendment (the six-class taxonomy table becomes seven; the `studios-typescript-native` stub steps and matrix note are added).
