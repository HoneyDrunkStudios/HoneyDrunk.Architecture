...
**Status:** Accepted
**Accepted:** 2026-06-06
**Amended:** 2026-06-13 - Curiosities may carry a lightweight lane tracker before it qualifies as a full multi-ADR Program.
**Date:** 2026-06-06
...
- **`roadmap.md`** — groups work by "product thread" (HoneyHub / NovOutbox / Curiosities) at the *quarterly horizon*. It answers "what quarter does this land in," not "what is the live dependency graph between this product's decisions right now."
- **`current-focus.md`** — the ranked, strict-ordinal priority list. A product surfaces here as one actionable row at a time, never as a whole multi-ADR effort. By design, `current-focus.md` rows are single phases, not rollouts.
...
These have real edges. The routing/telemetry ADR cannot be written until the bridge ADR is Accepted (it depends on the bridge's session contract). BYOK cloud-execution can't build until the local v1 ships and the §5 probe converts. The dependency is *between decisions* and *between phases*, and **no Grid construct holds it**. The same shape applies to **NovOutbox (PDR-0002)** — ADR-0027 node standup, tenant-isolation (ADR-0026/0050), Stripe/billing (ADR-0037), REST API (ADR-0057), client NuGet (ADR-0034/0035) — and to **Curiosities (PDR-0008)**. These are exactly the three named 2026 product threads on `roadmap.md`.
...
- **NovOutbox** → `initiatives/programs/notify-cloud.md`, governing PDR-0002; file path retained for link stability until a cleanup pass renames paths. ✅ Qualifies (ADR-0015 / ADR-0019 / multi-tenant primitives / ADR-0027 / Stripe).
- **Curiosities** (PDR-0008) → **lane tracker allowed before full Program qualification.** Per D1, a full Program still requires >1 ADR; PDR-0008's follow-ups are content/spike **packets** with only a *conditional* "future ADR" (backend storage / curated-content pipeline / editorial review queue, *if* accepted for build). However, the 2026-06-13 prioritization pass made Curiosities one of the three active product lanes beside HoneyHub and NovOutbox. A lightweight `initiatives/programs/curiosities.md` tracker is therefore allowed as a lane tracker so roadmap/focus/initiative surfaces have a single pointer. It becomes a full ADR-0089 Program if and when Curiosities spawns more than one ADR.
...
- **`roadmap.md` and `current-focus.md` gain a link target.** Each product lane on `roadmap.md` can now link to its tracker; current-focus rows can link to the relevant tracker for the full dependency picture. These are link additions, not schema changes.
- **Backfill work.** HoneyHub and NovOutbox are authored as the first full Program trackers. Curiosities may be authored as a lightweight lane tracker after the 2026-06-13 prioritization pass; it is not treated as a full multi-ADR Program until it spawns >1 ADR. HoneyHub first. Further program/tracker work can be delegated to the `scope` agent.
