...
## Amendment (2026-05-16) — `IAuditLog`/`AuditEntry` relocated to HoneyDrunk.Audit

*This is an additive amendment recorded per ADR-0030 D5 and ADR-0031. It does not change this ADR's Status or restate its decision content. The decisions above stand as written for everything except the audit contracts' ownership, which is amended here.*

ADR-0030 (Grid-Wide Audit Substrate) and ADR-0031 (Stand Up the HoneyDrunk.Audit Node) establish that durable, attributable Grid-wide security and action audit is a first-class concern homed in a **new dedicated `HoneyDrunk.Audit` Node**, not in Operator. The structural reason: Operator is a control plane that decides and acts (kill authority, gates, breakers), and the recorder must not be the actor — the direct sibling of this ADR's anchoring repo-invariant 5. As a result, the following amendments apply to this ADR's audit-related text:

- **`IAuditLog` and `AuditEntry` are relocated** out of `HoneyDrunk.Operator.Abstractions` and into `HoneyDrunk.Audit.Abstractions`, generalized from Operator-runtime-scoped to Grid-wide. Where D2, D3, and D9 of this ADR list `IAuditLog`/`AuditEntry` as Operator-owned contracts, read them as relocated to `HoneyDrunk.Audit.Abstractions` and consumed by Operator. The other eight Operator contracts (`IApprovalGate`, `ICircuitBreaker`, `ICostGuard`, `IDecisionPolicy`, `ISafetyFilter`, `CostEvent`, `ApprovalRequest`, `ApprovalDecision`) are unaffected and remain Operator-owned.
- **Operator is reclassified from owner to consumer/emitter** of the audit contracts. Operator continues to record its own AI-runtime decisions exactly as D9 describes (every agent execution, tool invocation, inference call, approval decision, cost event, breaker transition), but it does so by emitting `AuditEntry` against the generalized `IAuditLog` it now *consumes* from `HoneyDrunk.Audit.Abstractions`, not against a contract it owns.
- **D9's "consumers work off the read surface, not by mutating entries" is satisfied by `IAuditQuery`**, the new read/forensics contract introduced in `HoneyDrunk.Audit.Abstractions` by ADR-0030. D9's append-only-by-interface guarantee is preserved and is now enforced at the Audit Node's `IAuditLog` surface.
- **Catalog and canary follow-through:** the `honeydrunk-operator` `catalogs/contracts.json` entries for `IAuditLog`/`AuditEntry` are marked relocated to `honeydrunk-audit`; the contract-shape canary for these two moves to the Audit Node's CI (ADR-0031 D8). This ADR's D10 canary set (`IApprovalGate`, `ICircuitBreaker`, `ICostGuard`, `ISafetyFilter`) is unchanged — `IAuditLog`/`AuditEntry` were never in this ADR's canary set.

See ADR-0030 (driving decision) and ADR-0031 (Audit Node stand-up) for the full rationale, the phased-integrity scope discipline, and the honest tamper-evidence limitation.
