...
This ADR is the decision record. It files no packets. The teardown work decomposes into the following packets (to be generated by the `scope` agent after operator review), ordered to honor D3's groups and the invariant-103 gate:

1. **`packet: openclaw-decommission/00-architecture-adr-0088-acceptance`** — flip ADR-0088 to Accepted; flip ADR-0081 to `Superseded by ADR-0088`; update `adrs/README.md` (ADR-0088 row, ADR-0081 status); remove ADR-0081 from `initiatives/proposed-adrs.md`. No teardown side effects. (Group 1 prerequisite verification is recorded in this packet's Required Decision.)
2. **`packet: openclaw-decommission/01-architecture-remove-openclaw-reference-files`** — remove `infrastructure/openclaw/grid-review-runner.md` and `infrastructure/openclaw/hive-sync.md`; update the `infrastructure/workers/grid-agent-runner/README.md` pointer. (Group 2 step 5.)
3. **`packet: openclaw-decommission/02-ops-teardown-runtime-and-tunnel`** — operator/human chore packet: remove OpenClaw Gateway / Honeyclaw / webhook bridge / OpenClaw-bound Cloudflare Tunnel hostname on the home server. Records the disable of OpenClaw schedules per migrated job. (Group 2 steps 2–4.)
4. **`packet: openclaw-decommission/03-ops-delete-org-secret`** — human-only org-admin chore: delete the `OPENCLAW_GRID_REVIEW_WEBHOOK_SECRET` org secret; close issue #527 (decommission, no successor issue). **Hard prerequisite for packet 04.** (Group 3 step 6 + step 9.)
5. **`packet: openclaw-decommission/04-architecture-retire-inventory-row`** — **gated on packet 03 confirming deletion** (invariant 103): remove the inventory row; remove/tombstone `openclaw-webhook-secret-rotation.md`; remove the matrix row from `constitution/node-standup.md`. (Group 3 steps 7–8 + Group 4 step 10.)
6. **`packet: openclaw-decommission/05-architecture-reconcile-adr-prose`** — reconcile OpenClaw references in ADR-0082 (invariant 102 enumeration, README row) and ADR-0083 (footprint prose) as documentation-currency edits. (Group 4 step 11.)
7. **`packet: openclaw-decommission/06-actions-remove-deprecated-inputs`** — in `HoneyDrunk.Actions`, remove the deprecated `openclaw-webhook-url` / `openclaw-webhook-secret` / fallback inputs from `job-review-request.yml` once no caller references them. Sequenced after packet 03. (Group 4 step 12.)

One PR per repo per the standing convention: packets 1, 4, 5 land in `HoneyDrunk.Architecture`; packet 6 in `HoneyDrunk.Actions`; packets 2 and 3 are operator/human chores. Packet 04 must not file or merge until packet 03 confirms the secret is gone.
