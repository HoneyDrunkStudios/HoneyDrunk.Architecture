...
This ADR is the decision record. The teardown work is **scoped and committed** in this PR under `generated/issue-packets/active/adr-0088-openclaw-decommission/` (dispatch-plan + 9 packets across Waves 0–4), produced via `adr-composer → scope → refine → re-scope` and ordered to honor D3's groups and the invariant-103 gate. `file-issues` files them after this ADR merges.
