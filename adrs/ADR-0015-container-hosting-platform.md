...
Recorded per ADR-0008 § Implementation-Notes Packets. Full as-built record (every delta + rationale, PRs, follow-ups) lives in this initiative's `implementation-notes.md` — under `generated/work-items/active/adr-0015-container-apps-rollout/` while in flight, and `generated/work-items/completed/adr-0015-container-apps-rollout/` once `hive-sync` moves the closed packets there (the Hive Sync "completed work items are moved to `completed/`" invariant — packet relocation and the `filed-work-items.json` rewrite are `hive-sync`'s job, not this PR's). The decision above is preserved as written. Headline deltas (decided ➜ as-built):
