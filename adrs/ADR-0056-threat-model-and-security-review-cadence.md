...
- **Critical-severity CVEs auto-create work items** via the GitHub Actions integration per ADR-0008's work-item authoring path. The work item is automatically assigned the `security` and `priority:critical` labels and routed to the operator's primary queue. The `scope` agent receives the work item for execution as it would any other work item.
