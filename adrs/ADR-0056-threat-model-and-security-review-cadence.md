...
- **Critical-severity CVEs auto-create issue packets** via the GitHub Actions integration per ADR-0008's issue-packet authoring path. The packet is automatically assigned the `security` and `priority:critical` labels and routed to the operator's primary queue. The `scope` agent receives the packet for execution as it would any other packet.
