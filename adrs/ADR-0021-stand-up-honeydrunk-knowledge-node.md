...
- **Shipping `Providers.InMemory` at stand-up without Azure AI Search or Postgres pgvector means no production-ready backend exists at first release.** Accepted: `InMemory` is enough to unblock Sim, Lore, Agents, and Evals on the contract surface, and the first production backend lands under its own work item once a real consumer needs it.
