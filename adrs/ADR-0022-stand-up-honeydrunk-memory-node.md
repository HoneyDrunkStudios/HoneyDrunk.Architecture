...
- **Shipping `Providers.InMemory` at stand-up without SQL Server or Cosmos DB means no production-ready backend exists at first release.** Accepted: `InMemory` is enough to unblock Agents, Flow, Sim, and Evals on the contract surface, and the first production backend lands under its own work item once a real consumer needs it.
