...
- [ ] Pulse packet — confirm the `429` counter metric with tags `tenant_id`, `endpoint`, `tier`, `outcome` per [ADR-0010](./ADR-0010-multi-backend-telemetry-and-observability.md); document the spike-by-tenant alarm threshold from D10
...
- **Audit and observability.** [ADR-0030](./ADR-0030-grid-wide-audit-substrate.md) committed the audit substrate but the rate-limiter rejection event is not yet a named emitter. [ADR-0010](./ADR-0010-multi-backend-telemetry-and-observability.md) makes 429s observable in principle but does not commit a tag convention.
...
2. **Per-tier override (middle precedence).** An App Configuration key per tier (e.g., `RateLimit:Notify:Free:RequestsPerMinute = 200`) read via the existing App Configuration plumbing (per [ADR-0005](./ADR-0005-secrets-and-configuration-management.md) and the feature-flag plumbing in [ADR-0055](./ADR-0055-feature-flag-and-progressive-rollout-strategy.md)). Lets the operator tune tier ceilings without a code change. Behind a feature flag per [ADR-0055](./ADR-0055-feature-flag-and-progressive-rollout-strategy.md) so the override can be toggled off in incident.
...
**Observability** (per [ADR-0010](./ADR-0010-multi-backend-telemetry-and-observability.md)):
...
- **HoneyDrunk.Communications** — when its public surface activates (per [ADR-0019](./ADR-0019-stand-up-honeydrunk-communications-node-and-notify-refactor.md)), it composes the Kernel extension on the same terms.
