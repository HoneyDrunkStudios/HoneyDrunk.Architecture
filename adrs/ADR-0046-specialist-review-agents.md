...
- **The four highest-leverage lenses are recognizable today.** CFO, accessibility, security, and AI safety each have concrete recurring needs that the generalist rubric only partially covers. Naming the roster while it's fresh prevents lens-by-lens improvisation.
...
This ADR codifies the **pattern** (specialist agents per lens, manual invocation at v1) and names the **initial roster of four** with their intended lenses. Per-agent definition files (`.claude/agents/cfo.md`, `.claude/agents/a11y.md`, `.claude/agents/security.md`, `.claude/agents/ai-safety.md`) land as follow-up packets. CI-triggered invocation and output-format / failure-mode discipline are explicitly deferred per the manual-first commitment.
...
### D2 — Initial roster: four specialist agents

The starter roster names the four lenses with the highest recurring leverage at this stage of the Grid:

| Agent | Lens | Primary concerns | When to invoke (v1: manual) |
|-------|------|------------------|------------------------------|
| **`cfo`** | Cost impact and vendor discipline | Free-tier-first reasoning, vendor relationship consolidation, recurring-spend justification, paid-tier defaults questioned, sunk-cost leverage | ADRs/PDRs touching infrastructure, dependencies, vendors, SKUs, or recurring costs; PRs adding new Azure resources or external service usage |
...
These four were chosen on three criteria:
...
Other lenses considered (privacy/GDPR, SRE, migration safety, anti-entropy, DX, performance, AI-cost) are explicit follow-up candidates per D9, not v1.
...
- **Cost discipline.** Four specialists × every PR × LLM cost = budget breach. Manual invocation keeps the population small.
...
- `cfo` — once or twice per month (each ADR/PDR draft that touches cost; the recent observability ADRs would have been the recent invocations).
...
Combined: ~5–15 specialist invocations per month at v1. At ADR-0044's per-PR cost model (~$0.50–2 per invocation), the marginal monthly cost is single-digit dollars. Rolls up under ADR-0044's $40–100/month cloud-review budget; no separate budget line.
...
- **Specialists deepen four of the twenty categories** with dedicated lenses:
  - `cfo` deepens category 17 (Product / Business Alignment — cost-awareness sub-bullet) and parts of category 6 (Performance / cost).
  ...
  - `ai-safety` deepens category 18 (AI / Agent-Specific) — same lens, more rigor.
...
Each of the four agents gets a dedicated follow-up packet to author its `.claude/agents/{name}.md` file. The packets land in priority order:

1. **`cfo`** first — immediate retroactive value (would have caught the ADR-0040/0045 first-draft cost overcommit); applicable to every infrastructure/vendor ADR in flight or queued.
...
3. **`ai-safety`** third — applicable as the AI-sector standup wave progresses; ADR-0017 through ADR-0025 acceptance reviews would benefit.
4. **`a11y`** fourth — zero immediate need (no UI work in flight); lands before the first consumer-app PR.
...
- **Roster expansion.** Privacy/GDPR, SRE, migration safety, anti-entropy, DX, performance, AI-cost are all real candidates. Each lands as a future amendment with the same justification structure D2 used.
...
- **Phase 3 (Month 2)** — Author `.claude/agents/ai-safety.md`. Invoke against AI-sector standup ADRs (ADR-0017 onward) as they move toward acceptance.
- **Phase 4 (When UI work starts)** — Author `.claude/agents/a11y.md`. Lands ahead of the first consumer-app PR; no immediate work otherwise.
...
- **HoneyDrunk.Architecture** (this repo) — gains four new agent definition files in `.claude/agents/` over time per D8 / D10. `constitution/agent-capability-matrix.md` updated to register each new agent.
...
- **Four new agent files** to author and maintain. Each is bounded scope (one lens, one rubric, one mandatory context-load list) and lower-cost-to-maintain than a broader agent.
...
- Author `.claude/agents/cfo.md` (Phase 1).
...
- Author `.claude/agents/ai-safety.md` (Phase 3).
- Author `.claude/agents/a11y.md` (Phase 4, when UI work starts).
...
### One generalist "specialist coordinator" agent that subsumes all four lenses

Rejected. A single agent that knows when to act as `cfo` vs `a11y` vs `security` vs `ai-safety` is just a generalist with a context-switching prompt. Specialists earn their keep by being **focused** — the prompt is tuned, the context-load is narrow, the rubric is deep on one thing. Conflating them loses the advantage.
...
Considered. `cfo` alone has the strongest retroactive justification (the ADR-0040/0045 pivot). Rejected because naming the pattern with one agent and adding others ad-hoc is exactly the sprawl risk D2's explicit-roster discipline is designed to prevent. Better to commit to four lenses up front with phased authoring than to backfill the roster every few months.

### Start with a larger roster (eight to ten specialists)

Rejected. Each agent has authoring and maintenance cost; the four-agent starter roster covers the highest-leverage lenses with the strongest body-of-practice. Privacy/GDPR, SRE, migration safety, anti-entropy, DX, performance, AI-cost are named candidates for future amendments — D9 makes the path explicit. Better to land four and amend than to commit to ten and have half of them sit unused.
