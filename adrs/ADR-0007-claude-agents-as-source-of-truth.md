...
## Retired Operational Addendum: OpenClaw Skills

ADR-0088 retired the OpenClaw runtime and the companion-skill maintenance rule on 2026-06-01. `.claude/agents/*.md` remains the source of truth for Claude Code and Copilot agent definitions, and no OpenClaw skill mirror is required for new or changed Architecture agents.

Historical OpenClaw skills may remain in old records, but they are no longer a live compatibility surface for this ADR. Reintroducing any paired-skill requirement would require a new ADR because ADR-0088 deliberately removed OpenClaw as a Grid substrate.
