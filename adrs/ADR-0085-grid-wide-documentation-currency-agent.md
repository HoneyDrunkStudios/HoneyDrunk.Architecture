...
**Execution surface:** OpenClaw scheduled trigger (consistent with `hive-sync`), with manual dispatch supported. The schedule decision (D6) honors [ADR-0043](./ADR-0043-continuous-backlog-generation-strategy.md) D7's deferral pattern — manual cadence is the floor, automation is the optimization.
...
- **Authorship value: `agent-claude-code`.** The agent runs as a Claude Code execution surface invoked by OpenClaw; expanding the `pr-core.yml` enum to add `agent-docs-sync` would be a coordinated change across HoneyDrunk.Actions and every consumer pr-core gate, which is disproportionate to the value. `agent-claude-code` is accurate (Claude Code is the execution surface) and already supported.
...
**Execution surface: OpenClaw scheduled trigger, with manual dispatch.** Consistent with `hive-sync` per [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md). ADR-0043 D7's deferral pattern is honored: until OpenClaw scheduling for the Friday slot lands, the agent is invoked manually on the Friday cadence. The manual cadence is the floor.

**No GitHub Actions cron alternative is committed in this ADR.** A future ADR may reverse this if OpenClaw availability becomes a constraint; the agent itself is execution-surface-agnostic (it reads files, writes files, and calls `gh`), so the reversal cost is bounded.
...
- Wire OpenClaw scheduled trigger (Friday slot).
...
- Verifies: OpenClaw can invoke the agent, the App token mints correctly, the report format is useful, the false-positive rate on accuracy checks is acceptable.
...
- One PR per affected repo per run, with the `Authorship: agent-claude-code` + OOB-reason metadata.
...
- Confirm Friday cadence and OpenClaw integration. If 90-day observation shows weekly is too aggressive or too lax, adjust here.
...
### Decide the execution surface (OpenClaw vs GitHub Actions cron) in this ADR

Rejected per D6 deferral pattern, with a 90-day observation window written in (Phase 6 cadence finalization). The deferral is bounded. If OpenClaw availability proves to be the constraint, the agent runs manually for the bounded period and the cadence decision in Phase 6 chooses GitHub Actions cron.
...
Considered for v1; deferred to Phase 6. Adding a new enum value requires a coordinated change across `HoneyDrunk.Actions/.github/workflows/pr-core.yml` and every consumer pr-core gate plus the post-merge audit workflow. Using `agent-claude-code` (which is accurate — Claude Code is the execution surface) with `Out-of-band reason:` pointing at the per-run report path is operationally equivalent for v1 and avoids coupling this ADR's acceptance to a HoneyDrunk.Actions change. Phase 6 revisits in light of observed PR volume.
...
- [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md) — OpenClaw execution surface
