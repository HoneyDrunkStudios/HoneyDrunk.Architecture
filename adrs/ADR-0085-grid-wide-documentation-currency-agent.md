...
- **Authorship value: `agent-claude-code`.** The agent runs through Claude Code under the ADR-0086 runner; expanding the `pr-core.yml` enum to add `agent-docs-sync` would be a coordinated change across HoneyDrunk.Actions and every consumer pr-core gate, which is disproportionate to the value. `agent-claude-code` is accurate (Claude Code is the execution surface) and already supported.
