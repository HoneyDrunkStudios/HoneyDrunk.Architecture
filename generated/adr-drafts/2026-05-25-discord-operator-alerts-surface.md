...
The forcing function is [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) (External-SaaS Credential Inventory and Rotation Procedure, drafted today). [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D3 originally picked "GitHub issues with the `external-credential-rotation` label" as the rotation-tracking surface — and that choice survives in [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) as the canonical *tracking* surface (because closed issues are an audit trail and AI agents can walk them). But [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D5's escalation cadence (T-30 / T-7 / T+0) explicitly needs a higher-saliency channel than "another label-filtered issue in a 350-issue backlog." The operator named chat-webhook (Discord specifically) as the right alerting surface during ADR-0083 drafting — but Discord's structure across the Grid had not been decided. This ADR closes that gap and unblocks [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D3's final shape.
...
- **Not the home-server's job.** [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md) makes the home server the always-on local automation host. Native desktop notifications from home-server-hosted automation (OpenClaw, the [ADR-0044](./ADR-0044-grid-aware-cloud-code-review-and-ai-authored-pr-discipline.md) webhook bridge) are a **complementary** surface — useful when the operator is at the workstation, blind when the operator is mobile. Discord is the mobile-and-desktop, anywhere-the-operator-is surface. The two are additive, not substitutes; routing to both is allowed and sometimes desirable, but Discord is the source-of-truth alerting surface.
...
**Inventory and rotation.** Each Discord webhook URL is an external-SaaS credential per [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) and lands in `infrastructure/reference/external-credentials.md` as seven inventory rows. Discord webhook URLs have **no provider-imposed expiration** — they remain valid until manually revoked. The inventory rows therefore carry `Expiration Cadence: n/a — non-expiring (rotate on suspected compromise only)` and have no standing rotation issue, but they still appear in the inventory because they are external-SaaS credentials by every other property: bound to a Discord-resource principal, stored as GitHub org secrets, with a real blast radius if leaked. Rotation procedure per [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D4 lands as `infrastructure/walkthroughs/discord-webhook-rotation.md` (regenerate the webhook in Discord → update the GitHub org secret → smoke-test). This composes naturally with [ADR-0083](./2026-05-25-external-saas-credential-rotation.md); it does not amend it.
...
- **Azure Key Vault as the storage backend.** Considered. Rejected because GitHub Actions workflows are the dominant emitter (CI failures, hive-sync, nightly workflows, release pipelines, [ADR-0044](./ADR-0044-grid-aware-cloud-code-review-and-ai-authored-pr-discipline.md) review-pipeline), and GitHub-Actions-to-Key-Vault adds an OIDC round-trip per workflow. The credentials are Actions-secrets-shaped (per [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D1 categorization), not workload-runtime-shaped. They live in GitHub org secrets alongside `SONAR_TOKEN`, `NUGET_API_KEY`, `GH_ISSUE_TOKEN` for the same reason those do.
...
| **GitHub Actions: scheduled-workflow failure** | `nightly-deps.yml`, `nightly-security.yml`, `hive-field-mirror.yml`, `weekly-governance.yml`, `external-credentials-check.yml` ([ADR-0083](./2026-05-25-external-saas-credential-rotation.md)), grid-health aggregator ([ADR-0012](./ADR-0012-grid-cicd-control-plane.md) D6) | Cron job fails or produces a drift finding |
| **[ADR-0083](./2026-05-25-external-saas-credential-rotation.md) credential-rotation escalation** | `external-credentials-check.yml` | T-30 / T-7 / T+0 against inventory |
| **[ADR-0044](./ADR-0044-grid-aware-cloud-code-review-and-ai-authored-pr-discipline.md) review pipeline** | `job-review-request.yml` (or its successor in the home-server-hosted bridge per [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md)) | Review verdict posted on a PR |
...
| **GitHub: PR opened / merged** | GitHub repository webhook → tiny relay (home server per [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md)) **or** GitHub-Actions-on-`pull_request` reusable workflow | PR lifecycle events |
...
| Credential rotation escalation T-30 ([ADR-0083](./2026-05-25-external-saas-credential-rotation.md)) | `#ops-alerts` | Medium | `🔑 {credential} expires in 30 days — {rotation-walkthrough-link}` |
| Credential rotation escalation T-7 ([ADR-0083](./2026-05-25-external-saas-credential-rotation.md)) | `#ops-alerts` + `#security-alerts` | High | `🔑⚠️ {credential} expires in 7 days — {rotation-walkthrough-link}` |
| Credential rotation T+0 (expired) ([ADR-0083](./2026-05-25-external-saas-credential-rotation.md)) | `#security-alerts` + `#audit-sensitive` | Critical | `🔑🔥 {credential} EXPIRED — {incident-record-link}` |
...
- `channel` (one of: `ops-alerts`, `security`, `agent-activity`, `hive-activity`, `release`, `announcements`, `audit-sensitive`) — selects which `DISCORD_WEBHOOK_*` secret to use
...
For emitters **outside** GitHub Actions (the home-server-hosted [ADR-0044](./ADR-0044-grid-aware-cloud-code-review-and-ai-authored-pr-discipline.md) bridge, scheduled local automations per [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md)), a small shared helper script (TBD shape, likely a one-file Python/Node/PowerShell that mirrors the workflow's contract) lives in `HoneyDrunk.Architecture/infrastructure/scripts/discord-notify.{ext}` and reads webhook URLs from local secret storage (per [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md) D5's machine-secret discipline). Same contract, same redaction rules, two delivery paths — GitHub Actions for cloud-CI emitters, home-server helper for local emitters.
...
- **Seven channels and seven secrets is real inventory.** Seven rows in [ADR-0083](./2026-05-25-external-saas-credential-rotation.md)'s `external-credentials.md`. Manageable, but not free.
...
- **A reusable workflow plus a home-server helper script is two delivery paths.** D9 calls this out — two seams, same contract, twice the maintenance touch on emitter changes. Accepted because GitHub Actions and the home server are the two distinct execution environments; a single seam would require centralizing everything through one or the other and neither is the right answer.
...
- **`HoneyDrunk.Vault.Rotation`** — **unchanged**. Discord webhook URLs are external-SaaS credentials per [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D1's CI/ops-machinery category, not runtime workload secrets; rotation stays manual via the [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) walkthrough, not via Vault.Rotation.
...
- **Home server per [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md)** — gains the `infrastructure/scripts/discord-notify.*` helper for local emitters, plus storage of the relevant `DISCORD_WEBHOOK_*` secrets per the machine-secret discipline in [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md) D5.
...
- `infrastructure/reference/external-credentials.md` gains seven new rows (one per Discord webhook) — landed alongside or after the [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) initial-inventory packet.
...
- [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D3's escalation language is amended (in follow-up, not in the ADR-0083 body — Proposed-ADR discipline) to cite this ADR as the alerting surface for T-30 / T-7 / T+0.
...
- **[ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md)** — home server hosts the second delivery path per D9; native desktop notifications are a complementary surface to Discord, not a competitor (D1).
- **[ADR-0082](./ADR-0082-canonical-node-standup-procedure.md)** — D10 onboarding hook attaches to the standup procedure document.
- **[ADR-0083](./2026-05-25-external-saas-credential-rotation.md)** — escalation surface; Discord webhook URLs are inventory rows in `external-credentials.md`; D6 onboarding hook in [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) parallels D10 here.
...
- **Provision the seven webhook URLs** in Discord (Server Settings → Integrations → Webhooks → New Webhook per channel) and store each as a GitHub organization secret (`DISCORD_WEBHOOK_OPS_ALERTS`, `DISCORD_WEBHOOK_SECURITY`, `DISCORD_WEBHOOK_AGENT_ACTIVITY`, `DISCORD_WEBHOOK_HIVE_ACTIVITY`, `DISCORD_WEBHOOK_RELEASE`, `DISCORD_WEBHOOK_ANNOUNCEMENTS`, `DISCORD_WEBHOOK_AUDIT_SENSITIVE`).
- **Author `infrastructure/reference/external-credentials.md`** entries for the seven Discord webhooks per [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D2 — cadence `n/a — non-expiring (rotate on suspected compromise only)`, owner solo-dev, blast-radius prose per channel.
- **Author `infrastructure/walkthroughs/discord-webhook-rotation.md`** per [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D4 — regenerate webhook in Discord → update GitHub org secret → smoke-test via `job-discord-notify.yml` with a test payload.
...
- **Author `HoneyDrunk.Architecture/infrastructure/scripts/discord-notify.*`** — the home-server-side helper per D9 (script-language choice deferred to the implementing packet; likely PowerShell on the home server, with a Python alternative for cross-platform agent use).
...
- **Phase 1 rollout** — wire CI failure on `main`, release events, NuGet publishes, scheduled-workflow failures, and the [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) credential-rotation escalation to `job-discord-notify.yml`. One packet per emitter family.
...
- **Amend [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) D3 follow-up note** (in the follow-up packet, not in [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) body — Proposed-ADR discipline) to cite this ADR as the alerting surface.
...
Considered as the simplest-possible alternative. Rejected because email is the surface this ADR is built to *replace* — the operator's email already gets GitHub Actions failure notifications per [ADR-0012](./ADR-0012-grid-cicd-control-plane.md) D7, and that surface is exactly where alerts go to die alongside marketing email and product newsletters. The forcing function ([ADR-0083](./2026-05-25-external-saas-credential-rotation.md)'s "label-filtered GitHub issues still get lost in a 350-issue backlog") applies equally to "label-filtered emails get lost in an inbox." Chat is structurally different from email — the unread-indicator, the channel taxonomy, the mobile push behavior, the shared timeline are all properties email does not provide.

### Defer this ADR until [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) ships in production

Considered. Wait until [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) hits the field and the T-30 escalation has fired a few times against the existing GitHub-issues surface; if it works, no need for Discord. Rejected on the same grounds [ADR-0082](./ADR-0082-canonical-node-standup-procedure.md) and [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) themselves were drafted now rather than deferred: the gap is real today across multiple sources (CI failures, agent activity, security signals, release events, hive-sync drift), [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) is a forcing function but not the only one, and the cost of the ADR is low. Industrialize the operator-alert surface before [ADR-0083](./2026-05-25-external-saas-credential-rotation.md)'s first T-30 fires — the inventory shows SonarCloud's 60-day cap means a first rotation event is realistically within the next 30–60 days from acceptance, and a missed signal in that window collides exactly with what this ADR is built to prevent.
...
- [ADR-0081](./ADR-0081-home-server-for-openclaw-and-local-agent-infrastructure.md) — home server hosts the local-emitter helper; native desktop notifications are complementary
- [ADR-0082](./ADR-0082-canonical-node-standup-procedure.md) — onboarding hook attaches to the standup procedure document
- [ADR-0083](./2026-05-25-external-saas-credential-rotation.md) — forcing function; rotation escalation routes here; Discord webhook URLs are inventory rows
- [PDR-0002](../pdrs/PDR-0002-notify-as-a-service-first-commercial-product.md) — Notify Cloud community Discord is a sibling surface, channel-isolated
- [`generated/adr-drafts/2026-05-23-charter-aware-adr-and-node-candidates.md`](./2026-05-23-charter-aware-adr-and-node-candidates.md) — adjacent candidate-surface document (build-in-public alerting was named there as a candidate surface)
