...
```
Authorship: Agent
Packet: generated/issue-packets/proposed/adr-0081-home-server/02-architecture-home-server-os-hardening-playbook.md
```

If multiple Wave 1 packets land in one combined PR per the initiative's cadence rule, list the additional packets on separate `Packet:` lines (one per line) or use a single `Packet:` line pointing to the dispatch plan with the individual packet paths in the PR description.
