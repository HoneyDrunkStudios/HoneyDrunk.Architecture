...
**Dependencies:**
- `packet:03` — hard. The sampler composes into the `TracerProvider` builder seam `HoneyDrunk.Telemetry.Sink.AzureMonitor` exposes.
...
- Packet 05 also touches `Sink.AzureMonitor` — packet 05 is sequenced after this packet (it depends on `packet:04`). Land this packet first; packet 05 rebases on it.
