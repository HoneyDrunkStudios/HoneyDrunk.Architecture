...
- **`since` versions may be placeholders.** If packet 02 / packet 03 have not yet established the new `HoneyDrunk.Audit` and `HoneyDrunk.Auth` versions at execution time of this packet, set `since` to a placeholder (`"TBD"`) and note in the PR that the value is to be reconciled before catalog readout consumption. Alternatively, the executor may sequence: land packet 02 → land packet 03 → land packet 01 (in which case `since` is concrete). The dispatch plan permits either order — packet 01's `dependencies:` is `[packet:00]` only.
...
**Dependencies:**
- `packet:00` — ADR-0050 Accepted; the invariants are live before the catalog entries reference them.
