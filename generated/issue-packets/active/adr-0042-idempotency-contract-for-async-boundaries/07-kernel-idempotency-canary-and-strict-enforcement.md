...
dependencies: ["packet:04", "packet:05", "packet:06"]
adrs: ["ADR-0042"]
wave: 5
...
**Dependencies:**
- `packet:04` — `IdempotentMessageHandler<T>` is the type whose keyless path is flipped to strict.
- `packet:05` — Notify producers must emit keys before strict enforcement, or they get poison-lettered.
- `packet:06` — Communications producers must emit keys before strict enforcement.
