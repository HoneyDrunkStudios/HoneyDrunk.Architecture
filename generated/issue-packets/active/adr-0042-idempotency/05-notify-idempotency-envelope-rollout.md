...
**Dependencies:**
- `packet:04` — `IdempotentMessageHandler<T>` and `IGridMessagePublisher` ship in `HoneyDrunk.Kernel` `0.8.0`.
- (Soft) `packet:03` — the Cosmos `IIdempotencyStore` is needed for deployed composition and the InMemory store for tests; both ship together in packet 03. If packet 03's package versions are not yet available at execution time, flag and coordinate.
