...
dependencies: ["packet:11", "packet:13"]
...
**Dependencies:**
- `packet:11` — the `platform/` directory + root `bicepconfig.json` must exist.
- `packet:13` — the app-facing modules (the platform layer reuses the module conventions; serviceBusNamespace from packet 13 is consumed for the shared SB namespace).
