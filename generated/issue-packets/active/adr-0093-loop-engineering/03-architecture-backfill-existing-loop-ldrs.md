...
1. **`hive-sync` reconciliation loop** (ADR-0014) — trigger: Mon/Thu schedule + manual; inputs: `filed-packets.json`, GitHub/Hive issue state, ADR/PDR frontmatter, catalogs; synthesizer: the `hive-sync` agent; gate: PR review (human); feedback_sink: initiative tracking files, `board-items.md`, `proposed-adrs.md`, `drift-report.md`; stop: PR merged; autonomy_tier: A.
