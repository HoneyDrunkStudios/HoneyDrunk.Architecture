...
- **Invariant 27** — all projects in a solution share one version and move together. Packet 02 is the bumping packet: bump every non-test `.csproj` to `0.2.0` in one commit. Partial bumps are forbidden. Packets 03/04/05 append to the CHANGELOG only.
