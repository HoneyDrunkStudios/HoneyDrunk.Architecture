...
## Context
ADR-0075 D1 replaces Swagger UI with Scalar as the canonical in-product OpenAPI renderer. The ADR's Follow-up Work names this as the first packet: "Ship `Scalar.AspNetCore` in the Web.Rest reference middleware composition."
...
> **Invariant 26 — Issue packets for .NET code work include an explicit `## NuGet Dependencies` section.** This packet has it; `HoneyDrunk.Standards` analyzers remain on every project.
...
**Dependencies:**
- `packet:00` — soft. ADR-0075 should be Accepted before its tooling lands.
