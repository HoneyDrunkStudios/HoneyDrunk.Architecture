...
## Dependencies
- `packet:01` — the `infrastructure/openclaw/` directory removal (the addendum cites that directory; retiring the rule after the directory is gone keeps the text honest).
- `packet:02` — the OpenClaw runtime teardown (the companion-skill rule is honest to retire once the OpenClaw runtime it served no longer exists).
...
**Dependencies:**
- `packet:01` — `infrastructure/openclaw/` directory removal (the addendum cites it).
- `packet:02` — OpenClaw runtime teardown (the rule served that runtime).
