...
dependencies: ["packet:00", "packet:02", "packet:06", "packet:09"]
adrs: ["ADR-0048"]
wave: 4
...
**Dependencies:**
- `packet:00` — ADR-0048 must be Accepted so the annotation makes sense as policy compliance.
- `packet:06` — `Migrations/README.md` template is needed for the copy step. If 06 hasn't merged, the executor reads ADR-0048 D11 directly and authors the README content matching the template's specification.
- `packet:02` — the `database` specialist agent ideally reviews this PR. Soft dependency — if 02 hasn't landed, the generalist `review` agent runs its category 13 surface check; the depth review is deferred.
