...
# Add schema_evolution field to grid-health.json and Migration Coordination line to integration-points template

## Summary
Two catalog/template tasks from ADR-0048's Follow-up Work: (1) add a per-Node `schema_evolution` field to `catalogs/grid-health.json` with values `ef-core-migrations`, `cosmos-schema-on-read`, or `n/a`; (2) extend the per-Node `integration-points.md` template with a Migration Coordination line so Nodes whose migration ordering matters across other Nodes have a discoverable place to record it.
...
- `repos/{name}/integration-points.md` — extend each existing file with a `## Migration Coordination` section. The wording template is identical across files; the per-Node content fills in what (if anything) sequences against the Node's migrations.

## Proposed Implementation

### 1. `catalogs/grid-health.json` — `schema_evolution` field
Add the field to every Node entry. Values, taken from ADR-0048 D1/D7:
- `ef-core-migrations` — the Node holds a relational store evolved via EF Core Migrations per ADR-0048 D1. Applies to: Audit (when standup lands), Memory, Knowledge, Billing, Notify Cloud, any Node with `HoneyDrunk.<Node>.Data` carrying a `DbContext`.
...
- `honeydrunk-data` — `n/a` at the Node level (Data ships the *pattern* and provider packages, but Data itself doesn't host a persistent store; each consuming Node owns its store). Record `n/a` and note in `notes` that the per-consuming-Node migration framework is `ef-core-migrations` per ADR-0048 D1.
- `honeydrunk-notify` — `ef-core-migrations` (Notify's `DbContext` already carries one or two scaffold migrations per ADR-0048 Context).
...
- `honeydrunk-audit` (Seed) — `ef-core-migrations` (Audit's `HoneyDrunk.Audit.Data` will hold the `AuditEntry` table per ADR-0030/ADR-0031). Even though the Node is Seed and the table doesn't exist yet, the *committed posture* is `ef-core-migrations` so the field is forward-looking.
- `honeydrunk-memory`, `honeydrunk-knowledge` (Seed) — `ef-core-migrations` (relational layouts per their standup ADRs); revisit at standup time if a vector-native backing is chosen, which would shift to `cosmos-schema-on-read`.
...
- Any other Node entries — assign by inspecting the Node's repo: a `DbContext` and a `Migrations/` folder ⇒ `ef-core-migrations`; a Cosmos backing ⇒ `cosmos-schema-on-read`; neither ⇒ `n/a`.

Bump `_meta.schema_version` to `1.1` and update `_meta.updated` to today's date.

### 2. `repos/{name}/integration-points.md` — Migration Coordination section
Locate every existing `repos/HoneyDrunk.<Node>/integration-points.md` (a `Glob` of `repos/*/integration-points.md` returns the current set). Append a `## Migration Coordination` section to each file. The per-Node content fills in based on the Node's actual migration interactions:

```markdown
## Migration Coordination

**Migration framework:** `ef-core-migrations` | `cosmos-schema-on-read` | `n/a` (matches `catalogs/grid-health.json`'s `schema_evolution` field).
...
- **Kernel** — note the Cosmos idempotency dedup store follows ADR-0048 D7 schema-on-read; the schema-on-read documentation lives in `Migrations/README.md` (per packet 08, when it lands).
- **Notify** — note existing scaffold migrations are retroactively annotated per packet 07 (when it lands).
...
## Acceptance Criteria
- [ ] Every Node entry in `catalogs/grid-health.json` carries a `schema_evolution` field with value `ef-core-migrations`, `cosmos-schema-on-read`, or `n/a`
- [ ] `catalogs/grid-health.json` `_meta.schema_version` is bumped to `1.1` and `_meta.updated` reflects the edit date
- [ ] Every `repos/HoneyDrunk.<Node>/integration-points.md` file carries a `## Migration Coordination` section with the four sub-fields (framework, downstream consumers, upstream contracts, notes)
...
**ADR-0048 D1 — EF Core Migrations is the Grid-wide framework for relational stores.** Document-store schema-on-read (D7) is the only exception. The `schema_evolution` catalog field surfaces each Node's committed posture in one place.

**ADR-0048 D7 — Document stores follow schema-on-read.** Cosmos and future vector/document backings have no DDL; new fields added by writing them; reading code tolerates absence; backfill jobs are documented as `Migrations/Backfill-YYYYMMDD-{description}.md` runbooks.

**ADR-0048 D8 — Audit table specifics.** `AuditEntry` migrations: no column drops ever; no type narrowing ever; no `NOT NULL` additions; new columns always nullable; paired-table pattern (`AuditEntryV2` alongside) for breaking changes. Surfaced in Audit's `integration-points.md` Migration Coordination section.

**ADR-0048 Follow-up Work — catalog and template tasks.** "Add the `schema_evolution` field to `catalogs/grid-health.json` (drift task per ADR-0014 `hive-sync`)." "Update `repos/{name}/integration-points.md` template with the Migration Coordination line." Both delivered by this packet.
...
## Constraints
- **Schema-evolution values are restricted to the three named: `ef-core-migrations`, `cosmos-schema-on-read`, `n/a`.** Do not invent additional values (e.g. `flyway`, `none`, `to-be-decided`). If a Node's posture is genuinely unknown, use `n/a` and note the uncertainty in `notes`.
...
**Objective:** Add the `schema_evolution` field to `catalogs/grid-health.json` for every Node and the Migration Coordination section to every existing `repos/{name}/integration-points.md` template.
...
**Dependencies:**
- `packet:00` — ADR-0048 should be Accepted before its catalog footprint is recorded.

**Constraints:**
- `schema_evolution` values are exactly one of `ef-core-migrations`, `cosmos-schema-on-read`, `n/a` — no other values.
- Library-only Nodes are `n/a`, not blank.
- Bump `_meta.schema_version` to `1.1`.
- Migration Coordination section default is "None — migrations are Node-local."; only Nodes with real cross-Node coordination get detail.
