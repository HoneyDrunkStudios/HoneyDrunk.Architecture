...
- **`HoneyDrunk.Standards`** — packet 02 is a docs + header-comment change only. No `PackageReference` added/removed/version-shifted; per invariant 27 (test projects excluded from the solution version) and per invariant 12 (per-package CHANGELOG only for packages with actual changes), this packet:
...
No cross-initiative `{Repo}#N` placeholders to substitute — all dependencies are intra-folder `packet:NN` edges. The folder is safe to push to `main` as-is.
