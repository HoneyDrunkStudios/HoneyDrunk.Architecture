...
5. **NEW — Resolve managed PR labels** — derive Grid-owned labels from the PR title/body, linked packet body/frontmatter when available, ADR references, changed-file paths, authorship, and current config. Managed labels include `adr-*`, `tier-*`, `wave-*`, initiative labels, `meta`, `docs`, `ci`, `bug`, `enhancement`, `dependencies`, `chore`, and review-state labels. The resolver must preserve human labels outside the managed set.
