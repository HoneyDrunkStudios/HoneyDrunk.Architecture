...
6. **Complete.** On verdict post, remove `agent-review-in-progress` and add either `agent-reviewed` (no `Block` / `Request Changes` findings) or `changes-requested-by-agent` (one or more findings at those severities). The verdict body posts as a PR comment using the established Grid Review format (`Risk Level`, `Review Confidence`, `✅ Verdict`, and the full emoji section checklist) preserved from the pre-ADR-0086 runner path.
