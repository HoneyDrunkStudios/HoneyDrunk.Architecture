...
dependencies: ["packet:03", "packet:07", "packet:11"]
...
## Dependencies
- `packet:03` — runner framework and heartbeat/state metadata.
- `packet:07` — Phase-A review cutover; the review queue is meaningful after at least one repo uses it.
- `packet:11` — scheduled agent job migration records the job ids, output paths, and old-schedule status to surface.
