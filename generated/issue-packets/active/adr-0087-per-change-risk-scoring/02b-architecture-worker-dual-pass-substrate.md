...
- `infrastructure/workers/grid-agent-runner/lib/Agent.psm1`'s `Invoke-ReviewAgentPasses` runs a flat `foreach ($command in $JobSpec.AgentCommands)` loop with **no** risk-conditional second pass.
...
- Reading `double_review_required` from the queue comment, or any trigger wiring — that is packet 03. This packet builds the *capability*; it must be invocable by a simple boolean/parameter so packet 03 only has to flip the trigger.
- Any change to `job-review-request.yml` or the scorer (packet 02).
- The Invariant 53 enforceability flip (packet 03).
...
### 1. Second pass in `Invoke-ReviewAgentPasses` (Agent.psm1)
Today the function runs a single flat `foreach ($command in $JobSpec.AgentCommands)`. Restructure so the runner can execute **two independent review perspectives** — the dual Codex CLI + Claude Code CLI pass — when invoked in dual-pass mode, returning both findings sets to the caller for synthesis. Gate the second pass behind a parameter (e.g. `-DoubleReview` / a job-spec flag) so the default single-pass path is unchanged when the flag is false. **Do not wire the flag to the queue comment here** — packet 03 does that; here it is a plumbing parameter with a sensible default (single pass).
...
- [ ] The dual-pass capability is invocable via a simple parameter/flag so packet 03 only needs to wire the trigger — this packet does NOT read `double_review_required` or any queue-comment field.
...
- [ ] No change to `job-review-request.yml`, the scorer, or Invariant 53 (those are packets 02 / 03).
...
## Human Prerequisites
- [ ] After merge, deploy/restart the grid-agent-runner on the home-server host (Task Scheduler per ADR-0086 / ADR-0081, outside CI) so the live worker carries the new substrate. The substrate is dormant (default single-pass) until packet 03 wires the trigger, so this restart is not time-critical relative to packet 02, but the worker must carry it before packet 03's cutover.

## Dependencies
None within this initiative. Independent of packet 01 and packet 02 (it touches no catalog and no scorer). Relates to **ADR-0086** Follow-up Work ("Implement dual-pass synthesis in the worker") — completing this packet closes that item. Packet 03 depends on this packet AND packet 02.

## Agent Handoff

**Objective:** Build the dual-pass execution + synthesis + contrarian-fallback substrate in `grid-agent-runner` so an enforcement trigger has real machinery to fire. Do not wire the trigger (packet 03's job).
...
**Constraints:**
- **Build the capability, not the trigger.** Gate the second pass behind a parameter with a single-pass default. Do NOT read `double_review_required` or any queue-comment field here — packet 03 wires that. Keeping these separate preserves one-PR-per-repo sequencing across waves.
...
**Key Files:**
- `infrastructure/workers/grid-agent-runner/lib/Agent.psm1` (`Invoke-ReviewAgentPasses` — add the second pass behind a flag)
...
**Contracts:**
- The dual-pass capability's invocation parameter/flag is the seam packet 03 wires to `double_review_required`. Keep it a simple boolean-ish parameter with a single-pass default.
