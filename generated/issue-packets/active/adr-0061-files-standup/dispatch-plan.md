...
| 02 | [Add two new Files invariants (domain-meaning boundary + download shape) to the constitution](./02-architecture-files-invariants.md) | Architecture | 1 | Agent | packet:01 |
| 03 | [Create `HoneyDrunkStudios/HoneyDrunk.Files` public repo, branch protection, labels, OIDC, clone locally (human-only)](./03-architecture-create-files-repo.md) | Architecture (tracking issue) | 2 | Human | packet:01 |
| 04 | [Stand up `HoneyDrunk.Files` — solution, four packages, contracts, CI with canary, InMemory reference adapter, smoke tests](./04-files-node-scaffold.md) | HoneyDrunk.Files | 3 | Agent | packet:01, packet:02, packet:03 |
