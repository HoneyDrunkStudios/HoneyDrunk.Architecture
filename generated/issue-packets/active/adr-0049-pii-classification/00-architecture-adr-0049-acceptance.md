...
**The fix in this packet:** in the D5 table row for `Audit (AuditEntry.DataChange)`, replace `[PiiField(SensitivePii)] → [REDACTED:sensitive]` with `[PiiField(SensitivePii)] → REJECTED (per D6 + invariant 59); only the field-name and classification metadata may appear, never via the audit body`. Leave the `[PiiField(Pii)]` and `[PiiField(Pseudonymous)]` clauses in the same row unchanged. This is the minimum change to make D5 and D6 (and packet 05's runtime behavior) read the same. This amendment lands in the same PR that flips ADR-0049 to Accepted — no separate ADR-amendment ADR.
