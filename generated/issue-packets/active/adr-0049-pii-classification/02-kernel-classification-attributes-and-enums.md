...
2. **`PiiCategory`** — `public enum PiiCategory { Pii, SensitivePii, Pseudonymous }`. XML-doc each value with the D2 definition; `SensitivePii` doc must call out GDPR Article 9 special-category status and the `IAuditLog`-channel forbidden-entirely rule (invariant 83 from packet 00).
...
- [ ] All new public types/members have XML documentation; the `SensitivePii` enum doc-comment calls out GDPR Article 9 + invariant 83 (never appears in audit even as redaction-token)
