...
- **`file-packets.yml` lifecycle gotcha:** after these packets move through The Hive, hive-sync may move the source files from `active/` to `completed/` per invariant 37. A `git revert` only undoes content edits, not packet-file moves. If a revert is needed after lifecycle moves, restore the packet files manually as part of the revert PR.
