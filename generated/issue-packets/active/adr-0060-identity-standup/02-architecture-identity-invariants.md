...
- `packet:01` — Packet 01 of this initiative must merge so the `repos/HoneyDrunk.Identity/invariants.md` file exists with placeholders, and so the `## Audit Invariants` section's adjacency is settled (no concurrent constitution edits between this packet and packet 01).
...
**Dependencies:** `packet:01` (catalog registration + context folder + ADR-0050/Auth amendments). Must be merged to `main` before this packet's PR is authored.
