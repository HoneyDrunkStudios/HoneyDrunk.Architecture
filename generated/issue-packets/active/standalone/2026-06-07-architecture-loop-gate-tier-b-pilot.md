...
- its **output is semantic text** (issue packets) that a model-as-judge + schema scorer can evaluate — unlike the autonomous Build Loop (ADR-0093 D5), whose gate is tests/coverage (ADR-0032), not Evals; the first *Evals* gate belongs on semantic output;
...
After the ADR-0086 `backlog-strategic-scope` job generates a candidate packet:
...
2. **Threshold:** candidates scoring ≥ threshold land in `proposed/` for human triage; sub-threshold candidates are written to a `generated/issue-packets/rejects/` log **with their `EvalReport`** — culled work is auditable, never silently dropped.
...
Per the chosen option: author the packet-quality suite (or judge prompt), wire it into the `backlog-strategic-scope` runner job as the post-generation gate, add the `generated/issue-packets/rejects/` sink, and update the loop's LDR to Tier B. Keep the change additive — the loop still produces `proposed/` candidates; the gate only changes *which* candidates reach the operator.
...
- [ ] Generated candidates are scored before landing; sub-threshold candidates go to `generated/issue-packets/rejects/` with their `EvalReport`, not to `proposed/`.
