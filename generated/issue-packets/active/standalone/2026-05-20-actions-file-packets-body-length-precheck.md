...
`HoneyDrunkStudios/HoneyDrunk.Actions` — both fixes live in `scripts/file-packets.sh`. The reusable workflow `.github/workflows/file-packets.yml` does not need changes (the script's exit code semantics are already what the workflow consumes via `if: always()` on the manifest commit step).
...
In `scripts/file-packets.sh`, after the new-packet discovery pass (the part that builds the `NEW_PACKETS` array around line ~150–200) and **before** the per-packet filing loop:
...
for packet in "${NEW_PACKETS[@]}"; do
  if issue_url=$(file_one_packet "$packet" 2>&1); then
    ...
    echo "::warning::Failed to create issue for $packet: $issue_url"
...
The existing `if: always()` manifest commit in the workflow keeps the `filed-packets.json` updates for successful packets even when later packets fail. No workflow change needed.

## Affected Files

- `HoneyDrunk.Actions/scripts/file-packets.sh` — pre-flight body-length check + per-packet failure capture + summary.
...
- [ ] `scripts/file-packets.sh` runs a body-length pre-flight check over all `NEW_PACKETS` before entering the per-packet filing loop. Each packet's rendered body (frontmatter excluded) is measured against a `BODY_LENGTH_LIMIT=65536` constant.
...
- [ ] Manifest commit (`filed-packets.json` updates) still runs on `if: always()` from the workflow, capturing the successfully-filed packets even on partial failure.
- [ ] Manual smoke test: author a deliberately-oversized test packet (e.g., 70 KB body) under `generated/issue-packets/active/standalone/` in a throwaway branch, push, observe the workflow fails fast with the per-packet diagnosis (no `gh issue create` attempted), then delete the test packet.
...
- Do NOT change the workflow file (`.github/workflows/file-packets.yml`) unless the script's exit-code contract changes — and it shouldn't.
...
**Key files:**
- `HoneyDrunk.Actions/scripts/file-packets.sh` (the only file to edit)
- Reference: `HoneyDrunk.Actions/.github/workflows/file-packets.yml` (read-only, to understand the exit-code contract)
