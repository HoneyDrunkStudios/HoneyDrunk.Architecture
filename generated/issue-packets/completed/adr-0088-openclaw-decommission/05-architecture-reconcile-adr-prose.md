...
dependencies: ["packet:03", "packet:00a"]
...
## Dependencies
- `packet:03` — secret deletion (the prose reconciliation is honest only after the secret is gone).
- `packet:00a` — the `docs-sync` job spec (the ADR-0085 re-point to "the worker's `docs-sync` scheduled job, no manual-floor caveat" is honest only once the spec exists).
...
**Dependencies:**
- `packet:03` — secret deletion (the secret-surface reconciliations are honest only after deletion; the ADR-0084 runtime / ADR-0085 execution-surface re-homing is honest after packet 02's runtime teardown + packet 00's ADR-0081 supersession, both upstream of packet 03 in the chain).
- `packet:00a` — the `docs-sync` job spec (the ADR-0085 re-point to the worker's `docs-sync` scheduled job, no manual-floor caveat, is honest only once the spec exists).
