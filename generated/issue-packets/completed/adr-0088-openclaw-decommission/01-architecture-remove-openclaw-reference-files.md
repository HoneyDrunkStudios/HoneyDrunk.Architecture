...
This is a docs-only packet. `Actor=Agent`. It is filed in Wave 2 (after acceptance) but does **not** depend on the operator runtime teardown (packet 02) — removing a stale doc is independent of disabling the live process. The dependency is only `packet:00` (acceptance).
...
## Dependencies
- `packet:00` — ADR-0088 acceptance (the teardown decision must be live before reference files are removed).
