...
Create CI workflow files and migrate GitHub Release notes from static templates to changelog-driven generation in `HoneyDrunk.Notify`. No application code changes. No version bump.
...
ADR-0009 establishes the package scanning policy for the Grid. `HoneyDrunk.Actions` already provides the reusable workflows (`pr-core.yml`, `nightly-security.yml`, `nightly-deps.yml`) and the `release/generate-notes` composite action. This packet wires them up in `HoneyDrunk.Notify` and replaces the static release-note template in `publish.yml` with auto-discovered changelog extraction.
    ...
    steps:
      ...
      - name: Generate Release Notes
        ...
        with:
          ...
          product-name: HoneyDrunk.Notify
          product-description: 'Notification subsystem for the HoneyDrunk Grid -- multi-channel delivery (Email, SMS), durable queuing, template rendering, and observability.'
          nuget-packages: |
            HoneyDrunk.Notify.Abstractions
            HoneyDrunk.Notify
            HoneyDrunk.Notify.Hosting.AspNetCore
            HoneyDrunk.Notify.Queue.Abstractions
            HoneyDrunk.Notify.Queue.InMemory
          ...
          docs-url: 'https://github.com/HoneyDrunkStudios/HoneyDrunk.Notify#readme'

      - name: Create GitHub Release
        uses: softprops/action-gh-release@v2
        with:
          tag_name: ${{ needs.prepare.outputs.version-tag }}
          name: HoneyDrunk.Notify ${{ needs.prepare.outputs.version-tag }}
