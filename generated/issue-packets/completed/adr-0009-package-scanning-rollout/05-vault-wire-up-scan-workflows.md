...
Create CI workflow files and migrate GitHub Release notes from static templates to changelog-driven generation in `HoneyDrunk.Vault`. No application code changes. No version bump.
...
ADR-0009 establishes the package scanning policy for the Grid. `HoneyDrunk.Actions` already provides the reusable workflows (`pr-core.yml`, `nightly-security.yml`, `nightly-deps.yml`) and the `release/generate-notes` composite action. This packet wires them up in `HoneyDrunk.Vault` and replaces the static release-note template in `publish.yml` with auto-discovered changelog extraction.
    ...
    steps:
      ...
      - name: Generate Release Notes
        ...
        with:
          ...
          product-name: HoneyDrunk.Vault
          product-description: 'Secrets and configuration management for .NET 10 with multi-provider support (Azure Key Vault, AWS, File, Configuration, InMemory).'
          nuget-packages: |
            HoneyDrunk.Vault
      ...
      - name: Create GitHub Release
        ...
        with:
          ...
          name: HoneyDrunk.Vault ${{ needs.prepare.outputs.version-tag }}
