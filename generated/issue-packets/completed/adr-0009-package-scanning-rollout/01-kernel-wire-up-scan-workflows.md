...
Create CI workflow files and migrate GitHub Release notes from static templates to changelog-driven generation in `HoneyDrunk.Kernel`. No application code changes. No version bump.
...
ADR-0009 establishes the package scanning policy for the Grid. `HoneyDrunk.Actions` already provides the reusable workflows (`pr-core.yml`, `nightly-security.yml`, `nightly-deps.yml`) and the `release/generate-notes` composite action. This packet wires them up in `HoneyDrunk.Kernel` and replaces the static release-note template in `publish.yml` with auto-discovered changelog extraction.
    ...
    steps:
      ...
      - name: Generate Release Notes
        ...
        with:
          ...
          product-name: HoneyDrunk.Kernel
          product-description: 'Semantic OS layer for HoneyDrunk.OS providing context propagation, lifecycle orchestration, and configuration management for .NET 10.'
          nuget-packages: |
            HoneyDrunk.Kernel.Abstractions
            HoneyDrunk.Kernel
          docs-url: 'https://github.com/HoneyDrunkStudios/HoneyDrunk.Kernel#readme'

      - name: Create GitHub Release
        uses: softprops/action-gh-release@v2
        with:
          tag_name: ${{ needs.prepare.outputs.version-tag }}
          name: HoneyDrunk.Kernel ${{ needs.prepare.outputs.version-tag }}
