...
- Decide whether the drift is purely traceability drift or whether PDR-0001 still needs new downstream ADRs or issue packets.
...
- If actual gaps remain, create proposed packets under `generated/issue-packets/proposed/` for those gaps, with one target repo per packet and no `accepts:` field because PDR-0001 is already Accepted.
...
**Constraints:**
- ADR-0043 lifecycle rule: agent-generated backlog packets land in `generated/issue-packets/proposed/`, never directly in `active/`; a human is the only authority for the `proposed/` to `active/` transition.
...
- HoneyHub boundary rule: HoneyHub may suggest ADRs, but Architecture accepts them. Architecture owns ADR/PDR governance, catalogs, routing rules, and generated issue packets.
