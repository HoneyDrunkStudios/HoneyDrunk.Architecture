...
**Created:** 2026-05-26
**Landing zone:** `generated/issue-packets/proposed/` per ADR-0043 D3. Agents never self-promote to `active/`; the operator does that triage by hand.
...
**Packets 10–13 merge into ONE Actions PR at file time** per the operator's "one PR per repo per initiative" convention. Each packet remains a discrete issue on The Hive for tracking purposes (each phase's acceptance criteria are independently checkable), but the file-issues pipeline files them as four sibling issues that all link to one consolidated PR against `HoneyDrunk.Actions`. The PR body lists all four packet paths in its `Packet:` metadata (multi-packet PRs are supported via newline-separated `Packet:` entries). If the consolidated PR grows too large to review (operator's judgment at PR-authoring time), the PR may be split along natural seams — but the file-issues pipeline handles the four-into-one mapping as the default. This honors the operator's PR-cadence preference (memory item `[PR cadence: prefer follow-ups]` + `[User: Oleg / HoneyDrunk Studios] one PR per repo per initiative`).
...
- This initiative's `dependencies:` arrays use `packet:NN` references for intra-folder edges and `external_dependencies:` entries (with `{adr-0083-packet-NN-issue-number}` placeholders) for the ADR-0083 cross-initiative edges.
...
Filing is automated. Once the operator promotes this folder from `proposed/` to `active/`, on push to `main`, `file-packets.yml` in `HoneyDrunk.Architecture` files every packet in this folder as a GitHub issue in its `target_repo`, adds it to The Hive (project #4), sets the board fields from frontmatter, and wires `addBlockedBy` edges from the `dependencies:` arrays. No manual `gh issue create`.

No cross-initiative `{Repo}#N` dependencies in this folder's `dependencies:` arrays — every entry is a `packet:NN` reference, all of which resolve within this folder once filed. The ADR-0083 coupling lives in narrative; the ADR-0083 scope agent wires its side of the coupling when that initiative is scoped.
