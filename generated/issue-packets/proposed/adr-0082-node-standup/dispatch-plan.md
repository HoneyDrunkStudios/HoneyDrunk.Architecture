...
This initiative lands in `generated/issue-packets/proposed/adr-0082-node-standup/` (the ADR-0043 D4 landing zone). Per ADR-0043 the scope agent does not self-promote to `active/`. After human review of the proposed packets, the operator (or the file-issues agent following the human-review approval) moves the folder to `active/` at which point `file-packets.yml` in `HoneyDrunk.Architecture` files every packet in the folder as a GitHub issue in its `target_repo`, adds it to The Hive (project #4), sets the board fields from frontmatter, and wires `addBlockedBy` edges from the `dependencies:` arrays.

No manual `gh issue create` from the scope agent. PR bodies for each packet must carry strict `Authorship: <enum>` + exactly one of `Packet:` (pointing at the packet path) or `Out-of-band reason:` — free-form text breaks pr-core checks.
