...
## Filing
Filing is automated. Once these packets land on `main` under `proposed/adr-0081-home-server/`, they are **not yet** filed as GitHub issues — per ADR-0043, the human triages `proposed/` → `active/` before `file-packets.yml` picks them up. No `gh issue create` commands are emitted here.
