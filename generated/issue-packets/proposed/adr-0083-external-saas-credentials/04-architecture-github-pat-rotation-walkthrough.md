...
|-------|-------------|-----------|
| `GH_ISSUE_TOKEN` | GitHub org secret in `HoneyDrunkStudios` (Settings → Secrets and variables → Actions) | `file-packets.yml` triggers via packet manifest push; the workflow's `gh issue create` call returns 401 on a bad token |
...
- **`GH_ISSUE_TOKEN`**: the `file-packets.yml` workflow can no longer file new GitHub issues from packets in `generated/issue-packets/active/`. Packet filing stalls; The Hive misses new items per ADR-0008 D7.
...
**PR Body Metadata:**
- `Authorship: agent`
- `Packet: generated/issue-packets/proposed/adr-0083-external-saas-credentials/04-architecture-github-pat-rotation-walkthrough.md`
