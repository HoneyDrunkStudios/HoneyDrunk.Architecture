...
- `.claude/agents/docs-sync.md` — confirm the **fallback packet path** for both new categories: when a finding lands and is not deduped against an existing un-triaged packet, write a `generated/issue-packets/proposed/{YYYY-MM-DD}-{repo}-docs-{slug}.md` packet per the Phase-2 dedup discipline.
...
## Dependencies
- `packet:03` — **hard**. Phase 3 must be observed working before Phase 4 broadens the detection scope. The Phase-3 catalog-reference reasoning underlies the Phase-4 dependency-graph reasoning (same catalog-driven approach).
...
## Constraints
> **Invariant 24:** Issue packets are immutable once filed as a GitHub Issue. (Applied here: the fallback-packet dedup rule from Phase 2 is preserved — the agent does not edit existing un-triaged `proposed/` packets on subsequent runs.)
