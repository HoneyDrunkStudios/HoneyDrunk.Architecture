...
Filing is automated: pushing these packets to `main` under `generated/work-items/active/adr-0032-pr-validation-policy/` triggers `file-work-items.yml`, which creates each issue, adds it to The Hive, sets board fields from frontmatter, and wires `addBlockedBy` from the `dependencies:` arrays (`work-item:01` for every Wave 2 packet). No manual `gh issue create` / `addBlockedBy` is emitted here by design — the scope agent's job ends at writing the packets. The user triggers the file-issues step separately.
