...
Create CI workflow files and migrate GitHub Release notes from static templates to changelog-driven generation in `HoneyDrunk.Data`. No application code changes. No version bump.
...
ADR-0009 establishes the package scanning policy for the Grid. `HoneyDrunk.Actions` already provides the reusable workflows (`pr-core.yml`, `nightly-security.yml`, `nightly-deps.yml`) and the `release/generate-notes` composite action. This packet wires them up in `HoneyDrunk.Data` and replaces the static release-note template in `publish.yml` with auto-discovered changelog extraction.
    ...
    steps:
      ...
      - name: Generate Release Notes
        ...
        with:
          ...
          product-name: HoneyDrunk.Data
          product-description: 'Persistence layer for HoneyDrunk.OS with multi-tenant data access, repository patterns, and Entity Framework Core integration.'
          nuget-packages: |
          ...
          docs-url: 'https://github.com/HoneyDrunkStudios/HoneyDrunk.Data#readme'

      - name: Create GitHub Release
        uses: softprops/action-gh-release@v2
        with:
          tag_name: ${{ needs.prepare.outputs.version-tag }}
          name: HoneyDrunk.Data ${{ needs.prepare.outputs.version-tag }}
