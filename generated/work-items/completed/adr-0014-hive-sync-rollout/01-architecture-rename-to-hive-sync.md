...
```markdown
| **hive-sync** | OpenClaw Monday/Thursday schedule or manual OpenClaw dispatch | `gh` CLI issue states, `generated/work-items/filed-work-items.json`, GraphQL Hive board state, `catalogs/grid-health.json`, `catalogs/nodes.json`, `adrs/ADR-*.md` frontmatter, initiative files | Updated `initiatives/` files via PR (new branch per run), packet moves (active → completed), `board-items.md`, `proposed-adrs.md` | Create issues, modify `filed-work-items.json` entries (may update existing paths only), make architectural decisions |
```
...
```
| initiatives-sync | `generated/work-items/filed-work-items.json`, `catalogs/grid-health.json`, `catalogs/nodes.json`, `initiatives/` |
```
...
The verifier command for the agent executing this work-item:

```bash
git grep -n "initiatives-sync" -- ":(exclude).git/*"
```

After the propagation edit, the only remaining matches must be (a) inside `adrs/ADR-*.md` files (Accepted ADRs, immutable), (b) inside `generated/work-items/active/**/*.md` files filed before this rename (immutable per invariant 24), (c) inside `generated/work-items/retired/**/*.md` if any, and (d) any historical commit message in the git log (which `git grep` does not search). All other matches must be eliminated.
...
- [x] Invariant 24 preserved — no edits to existing filed packets under `generated/work-items/active/` or `generated/work-items/retired/`.
...
- [ ] `git grep -n "initiatives-sync"` returns matches only inside `adrs/ADR-*.md`, `generated/work-items/active/**/*.md` (filed before this packet), or `generated/work-items/retired/**/*.md`. No matches in `.claude/`, `.github/workflows/`, `constitution/`, `CLAUDE.md`, `AGENTS.md`, or `initiatives/`.
...
**Constraints:**
- **Invariant 24** (full text above) — do not edit any file under `generated/work-items/active/` or `generated/work-items/retired/`. Filed packets, including those that mention `initiatives-sync`, stay frozen.
