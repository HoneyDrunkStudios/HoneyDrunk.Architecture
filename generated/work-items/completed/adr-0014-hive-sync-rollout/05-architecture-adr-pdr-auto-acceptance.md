...
For each packet under `generated/work-items/active/**/*.md` and `generated/work-items/completed/**/*.md`, parse the YAML frontmatter and extract the `accepts:` field (if present and non-empty). Build an in-memory map:
...
For each file in both lists, read the first ~10 lines and extract the frontmatter fields (Status, Date, Sector). Also build the **implementing-packet index** described in Step 9a — for each packet under `generated/work-items/{active,completed}/`, parse the YAML frontmatter `adrs:` and `pdrs:` fields. Write the parsed result to `/tmp/decision-frontmatter.json` for use by Step 9.
```
