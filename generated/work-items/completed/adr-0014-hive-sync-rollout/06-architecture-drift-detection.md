...
1. All six ADR-0014 packet files are moved from `generated/work-items/active/adr-0014-hive-sync-rollout/` to `generated/work-items/completed/`. Basenames preserved (no collisions expected): `01-architecture-rename-to-hive-sync.md`, `02-architecture-packet-lifecycle.md`, `03-architecture-board-items-tracking.md`, `04-architecture-proposed-adrs-queue.md`, `05-architecture-adr-pdr-auto-acceptance.md`, `06-architecture-drift-detection.md`.
2. The dispatch plan moves to `generated/work-items/completed/adr-0014-hive-sync-rollout--dispatch-plan.md` (initiative-prefixed because the bare basename `dispatch-plan.md` would collide with future initiatives).
3. The empty initiative subdirectory `generated/work-items/active/adr-0014-hive-sync-rollout/` is removed.
...
5. The `filed-work-items.json` keys for all six ADR-0014 packets are updated to point at the `completed/` paths.
...
- [ ] **End-to-end validation criterion (post-merge, observed by the human reviewer):** after this PR merges and all six issues close, the next scheduled/manual OpenClaw `hive-sync` run produces a PR in which:
  ...
  5. The six packet files are moved to `generated/work-items/completed/` with `filed-work-items.json` keys updated.
