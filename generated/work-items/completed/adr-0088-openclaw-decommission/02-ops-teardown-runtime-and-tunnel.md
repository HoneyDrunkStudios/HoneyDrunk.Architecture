...
dependencies: ["work-item:00", "work-item:00a"]
...
## Dependencies
- `work-item:00` — ADR-0088 acceptance (the teardown decision must be live).
- `work-item:00a` — the committed `docs-sync` job spec on the ADR-0086 worker (so stopping docs-sync's OpenClaw schedule does not strand it with no scheduler).
...
- `work-item:00a` — the committed `docs-sync` job spec (stopping docs-sync's OpenClaw schedule is gated on it).
