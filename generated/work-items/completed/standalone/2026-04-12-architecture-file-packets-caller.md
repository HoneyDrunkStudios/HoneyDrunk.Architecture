...
Add `.github/workflows/file-work-items.yml` to `HoneyDrunk.Architecture`. This caller workflow triggers on pushes to `main` that touch `generated/work-items/active/**` and on `workflow_dispatch`, then delegates to the reusable `file-work-items.yml` in `HoneyDrunk.Actions`. This is the glue that makes merging a packet PR automatically file its issues.
...
### `.github/workflows/file-work-items.yml`
...
- `.github/workflows/file-work-items.yml` (new)
...
- [ ] Workflow file exists at `.github/workflows/file-work-items.yml`
- [ ] Merging a PR that adds a packet to `generated/work-items/active/` triggers the workflow automatically
