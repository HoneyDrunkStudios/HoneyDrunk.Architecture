...
The Actions#57 hardening introduced `HIVE_PROJECT_METADATA_JSON` as an env-var passed from `file-work-items.sh` into `hive-project-mirror.sh` (script lines 160–164 and 199–203 already handle the cached path). That works for *batch* invocations from `file-work-items.sh` because the parent script can resolve once and pass to every child invocation. It does **not** work for the per-issue `hive-field-mirror.yml` path — there is no parent caching layer because every issue event is its own workflow run.
