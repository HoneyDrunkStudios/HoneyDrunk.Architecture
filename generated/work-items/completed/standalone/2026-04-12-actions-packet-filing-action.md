...
Build a GitHub Actions reusable workflow in `HoneyDrunk.Actions` that reads merged work items from `HoneyDrunk.Architecture/generated/work-items/active/`, files them as GitHub Issues in their target repos, adds them to The Hive (project #4), and populates all custom fields inline. This closes the ADR-0008 D6 gap and renders D5 (auto-add) irrelevant for the primary packet flow.
...
### Reusable workflow: `.github/workflows/file-work-items.yml`

**Triggers (when called via `workflow_call`):**
- Inputs: `architecture-ref` (branch/SHA to checkout Architecture repo, default `main`), `work-items-dir` (default `generated/work-items/active`), `project-number` (default `4`), `project-owner` (default `HoneyDrunkStudios`)
...
2. Read `generated/work-items/filed-work-items.json` (manifest — created if absent)
3. For each `.md` file under `work-items-dir` not already in the manifest:
...
5. Commit updated `filed-work-items.json` back to Architecture repo (uses `hive-field-mirror-token` which has `contents:write` on Architecture)
6. Output a summary table: packet → issue URL → linked blockers

### Filed-packets manifest: `generated/work-items/filed-work-items.json`

Machine-written by the action. Never hand-edited. Path relative to Architecture repo root.

```json
{
  "generated/work-items/active/standalone/2026-04-12-actions-packet-filing-action.md": "https://github.com/HoneyDrunkStudios/HoneyDrunk.Actions/issues/23"
}
```

**Idempotency rule:** If a packet path already has an entry, skip it unconditionally. Do not create a second issue.

### Script: `scripts/file-work-items.sh`

Encapsulates the per-packet loop and dependency-linking pass. Called by the workflow. Accepts:
- `--work-items-dir` — path to `active/` directory
- `--manifest` — path to `filed-work-items.json`
...
```markdown
> [!NOTE]
> Filed from work item in [HoneyDrunk.Architecture](https://github.com/HoneyDrunkStudios/HoneyDrunk.Architecture).
> Work Item: `{relative_work_item_path}` · ADRs: {adrs} · Initiative: {initiative}
...
- `.github/workflows/file-work-items.yml` (new — reusable workflow)
- `scripts/file-work-items.sh` (new)

The caller workflow in `HoneyDrunk.Architecture` is a separate packet — see `2026-04-12-architecture-file-work-items-caller.md`.
...
- [ ] `filed-work-items.json` is committed back to Architecture repo after the run
...
- [ ] `scripts/file-work-items.sh` runs locally with env vars set
