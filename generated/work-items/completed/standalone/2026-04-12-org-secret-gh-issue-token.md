...
Create a GitHub fine-grained personal access token with `issues:write` on all `HoneyDrunkStudios` repos and store it as the org secret `GH_ISSUE_TOKEN`. This unblocks the D6 batch-filing action (`2026-04-12-actions-packet-filing-action.md`) and the Architecture caller workflow (`2026-04-12-architecture-file-work-items-caller.md`).
...
5. Permissions:
   ...
   - Repository permissions → Contents → **Read and write** (needed to commit `filed-work-items.json` back to Architecture repo)
...
- `2026-04-12-architecture-file-work-items-caller.md`
