...
Filing is automated. Pushing these packets to `main` under `generated/work-items/active/adr-0033-deploy-trigger-model/` triggers `file-work-items.yml` in HoneyDrunk.Architecture, which:
...
- **2026-06-01 as-built reconciliation** — implementation diverged materially from the scoped design; recorded in [`implementation-notes.md`](implementation-notes.md) (first instance of the ADR-0008 Implementation-Notes process) and as a `## Implementation Notes` section on ADR-0033. Packets 01/02/03 left immutable (invariant 24). Headline deltas: model became an ADO-style **staged approval-gate** (`tag → staging → gated prod`) with per-environment deploy jobs (not a single dynamic `resolve`-mapped env); **Functions** ships build-once same-artifact while containers rebuild-per-registry (same-artifact deferred on a registry-topology call); **packet 02's Worker filter was factually corrected** to include core `HoneyDrunk.Notify` + `Abstractions` + shared-source `ProviderSupport` (they are in the Worker's build closure); CHANGELOG criterion dropped (neither repo has a `CHANGELOG.md`); ADR refs retained in workflow comments (documented deviation from the no-ADR-numbers convention). Shipped: Notify #54, Pulse #42; control-plane fix Actions #181 (`upload-artifact@v6` path normalization). First real dev deploys proved the trigger logic; remaining failures are dev infra/RBAC gaps (→ ADR-0077).
