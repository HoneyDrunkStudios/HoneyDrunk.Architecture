...
dependencies: ["work-item:00", "work-item:01", "work-item:02", "work-item:07"]
...
**Dependencies:**
- `work-item:00` — invariant 91 / 92 cited.
- `work-item:01` — catalog entry for `ICostLedger` under `honeydrunk-kernel` already exists; this packet only removes the AI-side entry.
- `work-item:02` — `cost-budgets.json` cross-referenced by the playbook.
- `work-item:07` — `_format.md` cross-referenced by the aggregator playbook entry.
