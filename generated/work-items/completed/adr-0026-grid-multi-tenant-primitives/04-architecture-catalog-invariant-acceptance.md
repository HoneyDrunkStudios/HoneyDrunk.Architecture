...
The new checklist line goes inside the existing `## Quality Checks` checklist block (lines ~63-76 today, the bulleted `- [ ]` items between "Before finalizing an work item:" and "## Anti-Patterns"). Append:
...
- [ ] No catalog / constitution / ADR text uses an "Invariant 37" reference anywhere it should use the inlined invariant text. (Architecture-internal docs may cite invariants by number with a brief one-line gloss; the rule about no-number-only references applies to work items, not Architecture knowledge-base docs.)
...
> **Invariant 24 (Work Tracking):** Work items are immutable once filed as a GitHub Issue. **Why it matters here:** This packet (and packets 01–03) are immutable after filing. If post-acceptance refinements to ADR-0026 or the new invariant text are needed, write a new packet rather than editing this one.
...
> **Invariant 24:** Work items are immutable once filed as a GitHub Issue. After filing, state lives on the org Project board, never in the packet file. If requirements change materially post-filing, write a new packet rather than editing the old one.
