...
> **Invariant 26 (Work Tracking):** Work items for .NET code work must include an explicit `## NuGet Dependencies` section. **Why it matters here:** The section above explicitly states "no new package references" rather than omitting the section.
...
> **Invariant 32 (Code Review):** Agent-authored PRs must link to their packet in the PR body. **Why it matters here:** The PR body must include `> Work Item: <permalink-to-this-packet>` so the review agent uses it as the primary scope anchor.
