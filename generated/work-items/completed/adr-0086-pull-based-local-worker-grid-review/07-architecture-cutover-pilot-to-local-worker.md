...
dependencies: ["work-item:02", "work-item:03", "work-item:04", "work-item:05", "work-item:06"]
...
## Dependencies
- `work-item:02` — existing review-agent GitHub App audit + Vault credential verification (**hard** — the worker can't authenticate to GitHub without these).
- `work-item:03` — Worker source + Task Scheduler installation (**hard** — there's no runtime to handle the claim without the worker).
- `work-item:04` — Schema doc updated (soft — the `.honeydrunk-review.yaml` edit aligns with the schema; the workflow doesn't strictly require the doc but the operator does).
- `work-item:05` — `job-review-request.yml` rewritten to label+comment form (**hard** — the caller's input shape change depends on the reusable workflow's input shape change).
- `work-item:06` — Worker labels and managed PR labels fanned out (**hard** for stable behavior — packet 05's `gh label create --force` safety net works on first use but the pre-seed is the canonical path; the worker's label-swap depends on `agent-review-in-progress` existing on the repo when the worker tries to add it).
...
**Dependencies:**
- `work-item:02` (hard), `work-item:03` (hard), `work-item:05` (hard), `work-item:06` (hard); `work-item:04` (soft).
