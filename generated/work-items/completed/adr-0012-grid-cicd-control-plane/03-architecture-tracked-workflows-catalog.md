...
| `honeydrunk-architecture` | `[]` (only `file-work-items.yml` and `initiatives-sync.yml` exist; neither is on a `schedule:` trigger today — both are `workflow_dispatch`/`workflow_run`) |
...
- `file-work-items.yml`, `initiatives-sync.yml` — Architecture-only orchestration workflows; not scheduled, no canonical health surface need today.
...
- [ ] `pr.yml`, `validate-pr.yml`, `hive-field-mirror.yml`, `deploy*.yml`, `file-work-items.yml`, `initiatives-sync.yml` do not appear in any `tracked_workflows` array (none are scheduled).
