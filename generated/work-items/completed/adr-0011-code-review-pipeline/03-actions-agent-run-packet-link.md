...
## Summary
Amend the cloud agent-execution workflow `agent-run.yml` so that when an agent opens a PR for an work item, the PR body always contains a fully-resolved link to the packet file in `HoneyDrunk.Architecture`. This makes invariant 32 ("agent-authored PRs must link to their packet in the PR body") mechanically enforced for every agent-authored PR.

The packet uses two complementary mechanisms:
1. **Prompt-side instruction (soft, primary path).** When `work-item-path` is supplied, the workflow appends a structured "Work Item: <permalink>" instruction to the agent's prompt envelope so well-behaved agents include the line directly.
2. **Workflow-side post-hoc assert (hard, mechanical guarantee).** After the agent finishes, a new "Assert PR-body packet link" step locates any PR the agent opened on the run's branch and uses `gh pr edit` to ensure the `> Work Item: <permalink>` line is present in the PR body. The workflow is the source of truth — invariant 32 enforcement does not depend on the LLM remembering its instructions.
...
```yaml
      work-item-path:
        description: >-
          Optional path to the work item file (relative to HoneyDrunk.Architecture
          root, e.g. generated/work-items/active/adr-0011-code-review-pipeline/02-actions-job-sonarcloud-workflow.md).
          When set, the workflow injects a "Work Item: <permalink>" line into the
...
```
https://github.com/HoneyDrunkStudios/HoneyDrunk.Architecture/blob/<architecture-ref>/<work-item-path>
```

Use `${{ inputs.architecture-ref }}` (which already defaults to `main`) so the link tracks the same ref the workflow checked out. Computing it in shell is safer than constructing it in `${{ }}` interpolation when `work-item-path` is empty:

```yaml
      - name: Resolve packet link
        id: packet
        env:
          WORK_ITEM_PATH: ${{ inputs.work-item-path }}
          ARCH_REF: ${{ inputs.architecture-ref }}
        run: |
          set -euo pipefail
          if [[ -n "${WORK_ITEM_PATH}" ]]; then
            LINK="https://github.com/HoneyDrunkStudios/HoneyDrunk.Architecture/blob/${ARCH_REF}/${WORK_ITEM_PATH}"
            ...
            if [[ ! -f "${WORK_ITEM_PATH}" ]]; then
              echo "::warning::work-item-path '${WORK_ITEM_PATH}' was supplied but the file does not exist at the resolved Architecture ref. The PR will still be opened, but the packet link will 404. Verify work-item-path matches the on-disk path."
            fi
          else
...
Modify the existing **Resolve prompt** step (it currently assembles the agent or direct-prompt envelope and writes it to `$GITHUB_OUTPUT`) so that when `work-item-path` is supplied, the envelope ends with a structured instruction:
...
```yaml
      - name: Resolve prompt
        ...
        env:
          ...
          HAS_WORK_ITEM: ${{ steps.packet.outputs.has-packet }}
          WORK_ITEM_LINK: ${{ steps.packet.outputs.link }}
          WORK_ITEM_PATH: ${{ inputs.work-item-path }}
        run: |
          set -euo pipefail
          ...
          # The agent must include this exact "Work Item:" line in any PR body it opens.
          if [[ "${HAS_WORK_ITEM}" == "true" ]]; then
            PACKET_INSTRUCTION="

          ---

          **Work item for this run:** \`${WORK_ITEM_PATH}\`
          **Packet permalink:** ${WORK_ITEM_LINK}
...
#       with:
#         agent: execute
#         work-item-path: generated/work-items/active/adr-0011-code-review-pipeline/02-actions-job-sonarcloud-workflow.md
...
The prompt-injection in step C is best-effort soft enforcement. To make invariant 32 mechanically reliable regardless of LLM behaviour, add a new step **after** the agent run steps (after both `Run agent — Claude` and `Run agent — Codex`, conditional on `inputs.work-item-path` being non-empty) that locates the PR the agent opened on this run's branch and asserts the `> Work Item: <permalink>` line into the PR body via `gh pr edit`.

Step shape:

```yaml
      - name: Assert PR-body packet link
        if: ${{ inputs.work-item-path != '' }}
        env:
        ...
        run: |
          ...
          EXPECTED_LINE="> Work Item: ${WORK_ITEM_LINK}"
...
- **Idempotent.** If the agent's PR body already contains the exact `> Work Item: <permalink>` line (because the prompt-side injection worked), the step exits with no edit. If the line is missing, the step prepends a packet-link block and an `---` separator, preserving the agent's original body underneath.
...
- **The line format must match exactly.** `> Work Item: <permalink>` (greater-than sign, space, `Work Item:`, space, full URL). The grep is `-Fxq` (literal whole-line match) — no fuzzy matching. If the agent inserts a different format (`Work Item: <link>` without the quote, or `**Work Item:** <link>`, etc.), the step inserts the canonical form alongside the agent's variant. The review agent's parser keys on the canonical form, so both forms coexisting is harmless.

### F. No change to other callers

Do **not** edit any caller of `agent-run.yml` in this packet. The `work-item-path` input is optional with an empty default, so existing callers continue to work unchanged. Callers that pass a packet (the file-work-items driver, future packet-execution workflows) opt in by supplying the input.
...
## Acceptance Criteria
- [ ] `.github/workflows/agent-run.yml` declares a new optional input `work-item-path` with description and default `''`
- [ ] A new "Resolve packet link" step exists that computes the permalink (`https://github.com/HoneyDrunkStudios/HoneyDrunk.Architecture/blob/<architecture-ref>/<work-item-path>`) only when `work-item-path` is non-empty
- [ ] The "Resolve packet link" step warns (does not fail) when `work-item-path` is supplied but the file is missing at the resolved ref
- [ ] The existing "Resolve prompt" step is amended to append a structured packet instruction (the `> Work Item: …` quoted line + an `out-of-band` fallback hint) only when `work-item-path` is non-empty
- [ ] The amended prompt envelope produces the exact line `> Work Item: <permalink>` as a quoted markdown block — the review agent's packet-resolution logic depends on this format
- [ ] **A new "Assert PR-body packet link" step exists**, gated `if: ${{ inputs.work-item-path != '' }}`, that runs after the agent run steps. It locates the PR the agent opened on the current branch in `inputs.checkout-target` and asserts the `> Work Item: <permalink>` line into the PR body via `gh pr edit`. The step is the mechanical guarantor of invariant 32 — the workflow, not the LLM, is the source of the line.
...
- [ ] When `work-item-path` is empty, the workflow behaves identically to today (existing callers keep working — both the resolve and assert steps no-op).
- [ ] When `work-item-path` is supplied but invalid (file missing at ref), the resolve step logs `::warning::` and continues; the assert step still runs against whatever permalink was constructed.
- [ ] Header comment block contains a new usage example that demonstrates `work-item-path` and notes the post-hoc assert behaviour.
...
The first caller that actually exercises the new input is the next packet that ships the file-work-items / packet-execution driver — not in this initiative. Until then, the input remains optional and unused, which is the correct pre-rollout state.
...
- The review agent (local-only per ADR-0011 D10) gains a stable packet-link convention to parse — quoted markdown block starting with `Work Item: `.
- Future hardening (e.g. signed packet-link assertions, automatic `out-of-band` label application when `work-item-path` is empty) can layer onto the same step structure without re-architecting.
...
> **Invariant 24:** Work items are immutable once filed as a GitHub Issue. *(The link this workflow injects is to the packet file at the agent's checkout ref — stable for the duration of that execution. Post-merge, the file may be archived per ADR-0008 D10; the permalink continues to resolve via Git history.)*
...
- **The injected line format is a quoted markdown block: `> Work Item: <permalink>`.** The review agent parses this exact shape. Plain text or different bullet style breaks the contract. Both the prompt-side instruction and the post-hoc assert step must produce the same canonical form.
...
**Objective:** Add a `work-item-path` input to `agent-run.yml` plus two complementary mechanisms: (1) a prompt-envelope injection that instructs the agent to include the `> Work Item: <permalink>` line in the PR body, and (2) a post-hoc "Assert PR-body packet link" step that mechanically asserts the line into the PR body via `gh pr edit` after the agent finishes. The assert step is the load-bearing guarantor of invariant 32 — the workflow is the source of the line, not the LLM. Optional input (default empty); no failure on missing file or missing PR (notice/warn and continue); no permission changes; no caller changes.
...
- The injected line must be exactly `> Work Item: <permalink>` as a quoted markdown block — review agent parses this format. Both prompt-side and post-hoc-assert paths emit the same canonical form.
