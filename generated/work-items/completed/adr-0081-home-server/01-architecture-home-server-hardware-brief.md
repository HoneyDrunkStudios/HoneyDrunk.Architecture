...
```
Authorship: Human
Work Item: generated/work-items/proposed/adr-0081-home-server/01-architecture-home-server-hardware-brief.md
```

(`Authorship: Human` because this packet is `Actor=Human` — the operator authors the brief based on their procurement decision. If the operator delegates the markdown drafting to an agent after deciding the SKU, switch to `Authorship: Agent` and keep the same `Work Item:` line.)
