...
- **Strict PR body discipline.** PR body must carry `Authorship: <enum>` and exactly one of `Work Item:` / `Out-of-band reason:` — free-form text breaks pr-core checks. Use `Authorship: agent` (this packet is fully agent-authorable) and `Work Item: generated/work-items/proposed/adr-0084-discord-alerts/00-architecture-adr-0084-acceptance.md` (or its `active/` path once promoted).
...
- PR body must carry `Authorship: <enum>` and `Work Item: <path>` — no free-form text in the metadata fields.
