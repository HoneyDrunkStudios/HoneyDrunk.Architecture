...
## Dependencies
- `work-item:07` — authorship/pr-size checks (soft; `large-pr` is the label `pr-size-check` applies — seeding it before or alongside packet 07's landing avoids a window where the check applies a non-existent label).
...
**Dependencies:**
- `work-item:07` — authorship/pr-size checks (soft).
