...
dependencies: ["work-item:02b", "work-item:03b", "work-item:08", "work-item:15"]
...
## Dependencies
- `work-item:02b` - OpenClaw runner config and write-back/auth approach (**hard** - the audit step writes reports into `HoneyDrunk.Architecture` using the documented local `gh` auth or optional GitHub App path).
- `work-item:03b` - `job-review-request.yml` / runner trigger path (soft - shares the review request and runner contract).
- `work-item:08` - Grid-wide labels (**hard** - `audit-sample` must exist before the job applies it).
- `work-item:15` - `generated/post-merge-audits/` directory (**hard** - the audit report has nowhere to commit without it).
...
**Dependencies:**
- `work-item:02b` (hard - runner write-back/auth approach), `work-item:03b` (soft), `work-item:08` (hard), `work-item:15` (hard).
