...
dependencies: ["work-item:03", "work-item:04", "work-item:05"]
adrs: ["ADR-0044"]
accepts: ["ADR-0044"]
wave: 1
...
```
`skip_paths` is tuned for a docs/catalog repo — `generated/` (work items, drafts, audits) is excluded so the reviewer focuses on constitution, ADRs, catalogs, and agent definitions.
...
## Dependencies
- `work-item:03` — `job-review-agent.yml` (**hard** — the caller invokes it).
- `work-item:04` — `review.md` D3 rubric (**hard** — the reviewer must run against the twenty-category rubric for a meaningful Phase-1 quality signal).
- `work-item:05` — `.honeydrunk-review.yaml` schema doc (soft — the config is authored against the schema).
...
**Dependencies:**
- `work-item:03` — `job-review-agent.yml` (hard).
- `work-item:04` — `review.md` D3 rubric (hard).
- `work-item:05` — schema doc (soft).
