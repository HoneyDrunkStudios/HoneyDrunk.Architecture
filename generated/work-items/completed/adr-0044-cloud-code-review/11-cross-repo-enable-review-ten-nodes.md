...
dependencies: ["work-item:06b", "work-item:07", "work-item:08", "work-item:09", "work-item:10"]
adrs: ["ADR-0044"]
accepts: ["ADR-0044"]
wave: 2
...
## Dependencies
- `work-item:06b` - OpenClaw/Codex Phase-1 pilot + go/no-go (**hard** - Phase 2 does not start until Phase 1's exit criterion is met).
- `work-item:07` - authorship/pr-size checks in `pr-core.yml` (**hard** - Phase 2 makes authorship classification mandatory; the checks must exist).
- `work-item:08` - Grid-wide labels (**hard** - `large-pr` etc. must exist before `pr-size-check` runs in these repos).
- `work-item:09` - D3 rubric in the upstream authoring agents (soft - agent PRs raised in these repos should be authored against the same rubric the reviewer evaluates; the upstream-authoring discipline, not only the `Authorship:` line, should be live as the fan-out lands).
- `work-item:10` - execution-surface authorship emitters (soft - agent PRs in these repos should declare `Authorship:` automatically once the emitters land).
...
**Dependencies:**
- `work-item:06b` (hard), `work-item:07` (hard), `work-item:08` (hard), `work-item:09` (soft), `work-item:10` (soft).
