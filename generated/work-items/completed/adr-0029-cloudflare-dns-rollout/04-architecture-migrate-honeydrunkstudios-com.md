...
- `work-item:02` — `infrastructure/cloudflare-account-provisioning.md` (Cloudflare account exists with hardware-key 2FA, the API token convention is documented for future use, the lean record-comment scheme is documented for use by this packet's record-import step).
- `work-item:03` — `infrastructure/cloudflare-domain-transfer.md` (the generic per-domain procedure this packet executes, including the post-cutover smoke contract).
