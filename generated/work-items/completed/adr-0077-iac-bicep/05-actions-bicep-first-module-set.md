...
**Dependencies:**
- `work-item:02` — `acrhdbicep` registry exists and OIDC `AcrPush` is granted; required for the post-merge `modules/v1.0.0` publish to land somewhere.
- `work-item:03` — `bicep/modules/` directory tree and `bicepconfig.json` exist.
- `work-item:04` — `bicep-publish.yml` workflow exists, so the post-merge `modules/v1.0.0` tag fires the publish.
