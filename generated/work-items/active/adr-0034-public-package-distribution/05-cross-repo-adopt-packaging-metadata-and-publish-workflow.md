...
**This is a multi-repo packet** describing one repeated unit of work across the 11 repos in `target_repos`. The `file-work-items` agent files it as a tracking issue in `HoneyDrunk.Architecture` with one child issue per Node, or as 11 sibling issues. The per-repo work is identical in shape and small-to-medium.
...
## Dependencies
- `work-item:02` — the `HoneyDrunk.Standards` packaging-metadata + SourceLink fragment (**hard** — each repo imports it; it must exist).
- `work-item:03` — `job-publish-nuget.yml` (**hard** — each repo's release workflow is amended to call it; it must exist).
...
**Dependencies:**
- `work-item:02` (hard — the fragment), `work-item:03` (hard — the publish workflow).
