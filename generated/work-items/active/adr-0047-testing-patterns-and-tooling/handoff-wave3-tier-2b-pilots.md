...
2. **`10` — `HoneyDrunk.Data`: Tier 2b pilot.** Stand up `HoneyDrunk.Data.Tests.Integration.Containers` with a Testcontainers SQL Server container where the consuming Node uses SQL projects, or the provider-specific container required by a non-SQL Server backing. Cover repository round-trip behavior, the outbox append + dispatch-claim path, and at least one provider-specific behavior that justifies Tier 2b. Wire `job-integration-tests-containers.yml` into Data's `pr.yml`.
