...
**This is a multi-repo packet** describing one repeated unit of work across the two repos in `target_repos`. The work is identical in shape and small. The `file-work-items` agent files it as a tracking issue with two child issues, or as two sibling issues.
...
**Dependencies:**
- `work-item:00` — ADR-0039 acceptance (soft — references ADR-0039 D2 as a live rule).
