...
dependencies: ["work-item:01", "work-item:03", "work-item:04"]
adrs: ["ADR-0039"]
accepts: ["ADR-0039"]
wave: 3
...
**This is a multi-repo packet** describing one repeated unit of work across the 14 repos in `target_repos`. The `file-work-items` agent files it as a tracking issue with one child issue per repo, or as 14 sibling issues. The per-repo work is small and identical in shape.
...
## Dependencies
- `work-item:01` — `catalogs/nodes.json` `license` field (**hard** — step 1 reconciles each repo's `LICENSE` against this field; it must exist).
- `work-item:03` — FSL conversion of Notify/Communications (**hard for the Notify/Communications children** — those two repos' `LICENSE.md` must already be FSL before this packet confirms consistency; this packet does not touch their license file).
- `work-item:04` — `job-dco-signoff.yml` reusable workflow (**hard** — every repo's PR-validation workflow is amended to call it; it must exist).
...
**Dependencies:**
- `work-item:01` (hard — the `license` catalog field), `work-item:03` (hard for the Notify/Communications children — their FSL `LICENSE.md`), `work-item:04` (hard — the `job-dco-signoff.yml` workflow).
