...
- [x] **Superseded:** `work-item:02` originally provisioned a GitHub App + Anthropic key. Replaced by `work-item:02b`; do not execute the old filed issue.
- [x] `HoneyDrunk.Architecture`: Define the OpenClaw/Codex Grid Review Runner runtime - [`02b-architecture-openclaw-grid-review-runner.md`](02b-architecture-openclaw-grid-review-runner.md)
  - Blocked by: `work-item:01` (soft).
- [x] **Superseded:** `work-item:03` originally built an Anthropic/Claude `job-review-agent.yml`. Replaced by `work-item:03b`; do not execute the old filed issue.
- [x] `HoneyDrunk.Actions`: Build `job-review-request.yml` - the GitHub trigger rail for the OpenClaw reviewer - [`03b-actions-job-review-request-workflow.md`](03b-actions-job-review-request-workflow.md)
  - Blocked by: `work-item:01` (soft), `work-item:02b` (**hard** - runner payload/schema).
- [x] `HoneyDrunk.Architecture`: Update `review.md` with the D3 twenty-category rubric execution detail - [`04-architecture-review-md-twenty-category-rubric.md`](04-architecture-review-md-twenty-category-rubric.md) - **`current-focus.md` priority #7 (review.md half)**
  - Blocked by: `work-item:01` (soft).
- [x] **Superseded:** `work-item:05` documented the old Sonnet/Opus/API schema. Replaced by `work-item:05b`; do not execute the old filed issue.
- [x] `HoneyDrunk.Architecture`: Author the OpenClaw-aware `.honeydrunk-review.yaml` v1 schema doc - [`05b-architecture-review-config-schema-openclaw.md`](05b-architecture-review-config-schema-openclaw.md)
  - Blocked by: `work-item:01` (soft).
- [x] **Superseded:** `work-item:06` enabled the old cloud/API reviewer. Replaced by `work-item:06b`; do not execute the old filed issue.
- [x] `HoneyDrunk.Architecture`: Enable the OpenClaw/Codex reviewer on the Architecture repo (Phase 1 pilot) - [`06b-architecture-enable-openclaw-review-phase-1-pilot.md`](06b-architecture-enable-openclaw-review-phase-1-pilot.md)
  - Blocked by: `work-item:02b` (hard), `work-item:03b` (hard), `work-item:04` (hard), `work-item:05b` (soft).
...
- [x] `HoneyDrunk.Architecture`: Roll the D3 rubric into the upstream authoring agents (`scope`/`adr-composer`/`pdr-composer`/`refine`/`node-audit`) - [`09-architecture-d3-rubric-upstream-agent-rollout.md`](09-architecture-d3-rubric-upstream-agent-rollout.md) - **`current-focus.md` priority #7 (upstream half)**
  - Blocked by: `work-item:04` (**hard** - same category names/numbering).
- [x] `HoneyDrunk.Architecture`: Amend execution-surface prompts - emit `Authorship:` line + surface the D3 authoring checklist - [`10-architecture-execution-surface-authorship-and-rubric.md`](10-architecture-execution-surface-authorship-and-rubric.md)
  - Blocked by: `work-item:04` (hard), `work-item:07` (soft).
- [ ] 10 live .NET Nodes (cross-repo, tracked from `HoneyDrunk.Architecture`): Enable the OpenClaw/Codex reviewer on the 10 remaining live .NET Nodes - [`11-cross-repo-enable-review-ten-nodes.md`](11-cross-repo-enable-review-ten-nodes.md)
  - Blocked by: `work-item:06b` (**hard** - Phase-1 go), `work-item:07` (hard), `work-item:08` (hard), `work-item:09` (soft), `work-item:10` (soft).
...
- [ ] `HoneyDrunk.Actions`: Activate D8 multi-perspective review for high-risk-Node PRs + flip `pr-size-check` `> 800` to auto-comment - [`14-actions-d8-multi-perspective-review.md`](14-actions-d8-multi-perspective-review.md)
  - Blocked by: `work-item:03b` (hard), `work-item:13` (**hard - D8 activation is explicitly gated on `review_risk_class` landing**).
...
- [ ] `HoneyDrunk.Actions`: Build the D9 `audit-sample` post-merge labeling + audit job - [`16-actions-audit-sample-post-merge-job.md`](16-actions-audit-sample-post-merge-job.md)
  - Blocked by: `work-item:03b` (soft), `work-item:08` (hard), `work-item:15` (hard).
- [ ] `HoneyDrunk.Architecture`: Wire `hive-sync` to detect D3↔agent-file drift - [`17-architecture-hive-sync-d3-drift-detection.md`](17-architecture-hive-sync-d3-drift-detection.md)
  - Blocked by: `work-item:04` (hard), `work-item:09` (hard).
...
```bash
PACKETS="generated/work-items/active/adr-0044-cloud-code-review"
