...
**Dependencies:** work-item:05 (`job-discord-notify.yml` must exist). Work Item:06 (`discord-notify.ps1`) is a dependency only if the review pipeline executes on the home server — packet 06 is in the same wave so the dependency is structurally satisfied if execution lands on home server.
