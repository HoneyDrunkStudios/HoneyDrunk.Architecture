...
- **Invariant 27** — all projects in a solution share one version and move together. Packet 02 is the bumping work-item: bump every non-test `.csproj` to `0.8.0` in one commit. Partial bumps are forbidden. Packets 04 and 07 (also Kernel) append to the CHANGELOG only.
