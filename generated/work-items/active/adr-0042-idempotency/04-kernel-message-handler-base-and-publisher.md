...
**Dependencies:**
- `work-item:02` — `IIdempotencyStore`, `IGridMessageEnvelope`, `IdempotencyKey`, `IdempotencyClaim`, `IdempotencyOutcome` ship in `HoneyDrunk.Kernel.Abstractions` `0.8.0`. Packet 04 shares the `HoneyDrunk.Kernel` solution with packet 02 — land 02 first or rebase onto its merge.
