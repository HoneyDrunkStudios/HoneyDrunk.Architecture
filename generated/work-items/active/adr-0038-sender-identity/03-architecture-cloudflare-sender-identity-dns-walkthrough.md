...
dependencies: ["work-item:00", "work-item:02", "work-item:09"]
...
**Dependencies:**
- `work-item:00` — soft. ADR-0038 should be Accepted before its DNS obligation is executed.
- `work-item:02` — hard. The ESP must be picked before the SPF include-domain and DKIM selector namespace can be written concretely.
- `work-item:09` — hard. The DKIM selector records come from the ESP account packet 09 provisions. Both packets are in Wave 3; packet 09 runs first.
