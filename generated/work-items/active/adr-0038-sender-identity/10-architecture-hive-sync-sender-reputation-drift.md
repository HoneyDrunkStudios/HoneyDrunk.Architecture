...
**ADR-0014 — Hive sync.** `hive-sync` reconciles board-item correspondence and drift; findings without a `filed-work-items.json` entry are mirrored onto The Hive via `initiatives/board-items.md` (invariant 38). The agent runs through OpenClaw scheduled/manual execution.
...
**Dependencies:**
- `work-item:01` — hard. The `sender_reputation_status` field must exist for `hive-sync` to reconcile it.
