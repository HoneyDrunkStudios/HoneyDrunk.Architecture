...
**No cross-initiative `dependencies:` edges from this initiative.** All packets here either depend on each other (`work-item:NN`) or stand alone. The ADR-0059 initiative — if it lands first — does not block any packet here, because the InMemory reference implementation lives in Kernel and does not need the Cache Node to exist. Conversely, ADR-0059's scaffold packet may reference the `HoneyDrunk.Kernel.Abstractions` version this initiative bumps to, but expressing that is ADR-0059's job, not this one's.
