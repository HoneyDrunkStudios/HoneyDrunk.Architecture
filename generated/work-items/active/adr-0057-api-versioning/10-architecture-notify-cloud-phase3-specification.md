...
But:
1. **ADR-0027 (Notify Cloud standup) is Proposed**, not Accepted. The `HoneyDrunk.Notify.Cloud` repo does not exist on disk; `catalogs/nodes.json` has no `honeydrunk-notify-cloud` entry; `file-work-items.yml` cannot file an issue against a repo that does not exist.
...
2. At the bottom of the specification document, add a **"Follow-up Notify Cloud packet" outline** — the executor-ready skeleton for the future filer to turn into a real work-item:
...
- `work-item:02` — provisioning playbook authored (referenced by the spec's docs-site section).
