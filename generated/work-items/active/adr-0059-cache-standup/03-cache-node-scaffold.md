...
- `work-item:02` — packet 02 of this initiative must be Done so the GitHub repo `HoneyDrunkStudios/HoneyDrunk.Cache` exists, branch protection is applied, labels are seeded, OIDC is wired, and the local working tree is cloned. `file-work-items.sh` cannot file this packet's issue against a repo that does not exist.
...
**Dependencies:** `work-item:01` (Architecture-side catalog and context-folder registration must be in place) and `work-item:02` (GitHub repo must exist and be cloned locally).
