...
dependencies: ["work-item:01", "work-item:02", "work-item:03", "work-item:04"]
adrs: ["ADR-0055"]
wave: 3
...
**Dependencies:**
- `work-item:01` — catalog entry for `honeydrunk-featureflags` lands before the standup so the Node is discoverable.
- `work-item:02` — `featureflags-v1.json` schema exists so consumers (and this Node's documentation) can reference it.
- `work-item:03` — App Configuration walkthrough extension applied so the runtime backing has a live App Configuration resource and seeded labels to wire against.
- `work-item:04` — `HoneyDrunk.Kernel.Abstractions` ships `IFeatureGate` and the new minor is published; this Node compiles against the new minor.
