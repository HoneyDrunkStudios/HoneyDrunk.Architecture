...
- **The existing ADR-0044 review-agent GitHub App is audited for reuse** (packet 02) under `HoneyDrunkStudios` with worker-path permissions `pull_requests: write` + `issues: write` + `contents: read`; installed on `HoneyDrunk.Architecture` for Phase A; private key + app-id + installation-id verified in `kv-hd-automation-dev` under `GitHub--AgentRunner--AppId`, `GitHub--AgentRunner--PrivateKey`, and `GitHub--AgentRunner--InstallationId`. The walkthrough at `infrastructure/walkthroughs/review-agent-github-app-local-worker.md` records the portal audit and regeneration procedure.
- **The portable scheduled agent runner is running on the home server** (packet 03). Source at `infrastructure/workers/grid-agent-runner/`. The `grid-review` Task Scheduler entry runs on startup/logon, repeats on the configured cadence (60–120 s recommended), restarts on failure, and refuses overlapping runs. The runner mints App-installation tokens from Vault for review operations, polls for `needs-agent-review` PRs Grid-wide, claims one at a time via label swap + queue-comment edit, runs `.claude/agents/review.md` under Codex CLI (and Claude Code CLI when D8 high-risk triggers), synthesizes independent findings into one verdict comment, and posts verdict + label transition. The runner spawns child CLI processes with `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` explicitly unset. D8 multi-perspective dispatch is wired; activation is gated on ADR-0044 packet 13 (`review_risk_class` catalog field) landing. D9 audit-mode dispatch is wired; activation is gated on ADR-0044 packets 15/16 landing. Job specs also exist for `hive-sync`, `lore-source`, `lore-ingest`, and `lore-signal-review`; those scheduled jobs are cut over in packet 11, not in Wave 2.
...
1. **Decommission the OpenClaw review-runner role** (packet 08) — mark the legacy `infrastructure/openclaw/grid-review-runner.md` as superseded with a banner pointing to the new authority; cross-link from the new runner README. **Operator portal work as Human Prerequisites:** remove the Cloudflare Tunnel hostname for review traffic; rotate the ADR-0044 webhook-signing secret out of Vault per ADR-0006; disable the OpenClaw review-runner cron/poll. **`hive-sync` and Lore schedules remain running until packet 11 records runner smoke tests and cutovers.** **`adrs/ADR-0081-*.md` is NOT edited** — that one-line edit belongs to ADR-0081's own acceptance/amendment cycle.
...
- **Non-review OpenClaw/Honeyclaw jobs are preserved until packet 11.** Operator verifies `hive-sync` / Lore schedules remain running before merging packet 08 unless a specific job already has a recorded runner cutover.
...
All 10 fan-out Nodes are enabled with `runner: local-worker` (Observe / Architecture / Studios / Vault.Rotation / Seed Nodes / library Nodes excluded); each repo's smoke-test PR ran end-to-end through the runner; the Cloudflare Tunnel review-traffic hostname is removed; the ADR-0044 webhook-signing secret is rotated out and logged in Log Analytics per invariant 22; the OpenClaw review-runner role is disabled; non-review schedules are still running or explicitly cut over. Then file Wave 3 (packet 11 — scheduled agent job migration).
