...
dependencies: ["work-item:02", "work-item:03", "work-item:05", "work-item:06"]
adrs: ["ADR-0043"]
accepts: ["ADR-0043"]
wave: 3
...
## Scope
- For each Proposed ADR in the ADR-0034 to ADR-0042 range that does **not** already have a scoped initiative folder under `generated/work-items/active/`, run a `scope` pass and write the resulting packets to `generated/work-items/proposed/`.
...
2. Cross-reference `generated/work-items/active/` for an existing `adr-00NN-*` initiative folder.
3. For each ADR with **Proposed status and no existing folder**: run a `scope` pass producing packets with `source: strategic`, `generator: scope` frontmatter into `generated/work-items/proposed/`, named `{YYYY-MM-DD}-{repo}-{description}.md` per ADR-0008 D10. These are dry-run packets — they reference a Proposed (not Accepted) ADR and stay in `proposed/` until the parent ADR is accepted.
...
## Affected Files
- `generated/work-items/proposed/*.md` (new — zero or more, depending on how many ADRs in range lack a folder)
...
- [ ] All produced packets are in `generated/work-items/proposed/`, none in `active/`
...
## Dependencies
- `work-item:02` — the `generated/work-items/proposed/` and `generated/briefings/` directories must exist.
- `work-item:03` — the `source`/`generator` frontmatter contract must be documented before packets are produced with those fields.
- `work-item:05` — the `hive-sync` Strategic trigger documents how Strategic passes are triggered in steady state; this kickoff is the first manual exercise of that path.
- `work-item:06` — `scope.md` must document the agent-cadence-Strategic-runs-write-to-`proposed/` behaviour before `scope` is run as the Strategic source.
...
**Objective:** Run the first Strategic-source pass against the Proposed ADR-0034–0042 batch, seeding `generated/work-items/proposed/` and writing a Phase-1 kickoff report.
...
- `work-item:05` — `hive-sync` Strategic trigger documented.
- `work-item:06` — `scope.md` `proposed/` behaviour documented.
...
- `generated/work-items/active/` (read-only, to detect already-scoped ADRs)
- `generated/work-items/proposed/*.md` (new output)
