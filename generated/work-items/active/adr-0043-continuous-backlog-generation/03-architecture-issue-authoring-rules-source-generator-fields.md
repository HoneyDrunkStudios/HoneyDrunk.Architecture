...
- [ ] The file states that `proposed/` is outside the `file-work-items.yml` `active/**` path filter, so `proposed/` packets cannot auto-file
...
## Dependencies
- `work-item:01` — ADR-0043 must be Accepted; this packet documents its D2/D3/D6 decisions as live rules. The human-only-promotion rule references invariant 78 added in packet 01.
