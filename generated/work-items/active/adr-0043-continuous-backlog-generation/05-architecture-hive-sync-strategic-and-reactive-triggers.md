...
## Summary
Amend the `hive-sync` agent definition with two new responsibilities ADR-0043 D4/D8 assign it: (1) the Strategic-source trigger — when `hive-sync` flips an ADR/PDR to Accepted, it records that a `scope` pass is owed for that decision; (2) the Reactive-source drift-to-packet conversion — drift items at severity ≥ medium become packets in `generated/work-items/proposed/`, with deduplication against existing `proposed/`/`active/` packets.
...
`hive-sync` Step 12 already produces `initiatives/drift-report.md`. Amend the agent definition so that drift findings at **severity ≥ medium** additionally become packets written to `generated/work-items/proposed/`. Implementation guidance:
...
- **Deduplication is mandatory** — before creating a packet, `hive-sync` checks `generated/work-items/proposed/` and `generated/work-items/active/` for an existing packet covering the same finding. ADR-0043 D4 calls deduplication "the single most important quality control" for reactive sources. State the dedupe key explicitly: the (drift category, item identity) pair, the same identity `hive-sync` already uses for First Surfaced date stickiness.
...
## Dependencies
- `work-item:02` — the `generated/work-items/proposed/` directory must exist before `hive-sync` is told to write packets into it.
- `work-item:03` — the `source`/`generator` frontmatter contract must be documented in `issue-authoring-rules.md` before `hive-sync` is told to emit those fields.
...
**Dependencies:**
- `work-item:02` — `proposed/` directory exists.
- `work-item:03` — `source`/`generator` frontmatter contract documented.
