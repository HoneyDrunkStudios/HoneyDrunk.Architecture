...
## Summary
Create `.claude/agents/database.md` — the specialist review agent ADR-0048 D13 commits — embedding the D2/D5/D6/D8/D9/D10/D12 checklists. The agent walks every PR that touches `HoneyDrunk.<Node>.Database/`, `Backfill/`, or any file referenced from a `DbContext`, enforcing the schema-evolution policy ADR-0048 commits. Authored per ADR-0046's specialist-agent pattern.

## Context
ADR-0048 D13 commits the Grid to a specialist `database` review agent: "Per ADR-0046, the Grid commits to specialist review agents for high-risk surface areas. Schema evolution is one of them. The `database` agent (added to `.claude/agents/database.md` per ADR-0046's pattern) walks every PR that touches `HoneyDrunk.<Node>.Database/`, `Backfill/`, or any file referenced from a `DbContext`."

The agent is **required** on every PR that adds or modifies SQL project schema files, backfill runbooks, or DbContext model mappings. ADR-0044 D3 category 13 (Data and persistence integrity) yields the per-PR review surface from the generalist `review` agent; ADR-0048 D13 commits the specialist agent that owns the depth.
...
**Manual invocation at v1 per ADR-0046 D3.** The specialist is invoked manually by the operator (via Claude Code or the cloud-wired review pipeline with the agent named explicitly). No CI trigger. ADR-0048 D13 says the `database` agent is "required on every schema-touching PR" — at v1 the *operator* enforces that by invoking the agent; later automation (a CI-triggered specialist invocation) is a deferred follow-up scoped under ADR-0046 D9 if and when that ADR's D9 is exercised.

**Scope: triggers on `HoneyDrunk.<Node>.Database/`, `Backfill/`, or any file referenced from a `DbContext`.** The agent's invocation surface includes: SQL project files under `*/HoneyDrunk.*.Database/`, backfill runbooks under `*/Backfill/`, and any `.cs` file that is part of the `DbContext`'s model (entity types, `OnModelCreating` configuration, value-converter definitions). The agent does NOT trigger on application-layer changes that merely *use* the DbContext without modifying it.
...
1. **Author `.claude/agents/database.md`.** Structure follows the existing `.claude/agents/*.md` pattern. Required sections:
   ...
   **Frontmatter:**
   ...
   ---
   name: database
   description: Specialist review agent for schema evolution, DACPAC deployment, and persistence integrity per ADR-0048. Required on every PR that touches SQL project schema files, Backfill/, or any file referenced from a DbContext.
   model: opus
   ...
   **Body sections (match the format of `review.md` and the in-flight `cfo`/`security` agent files):**
   ...
   - **## Scope** — the agent reviews schema-touching PRs only; out-of-scope is application-layer DbContext usage that doesn't modify the model.
   - **## When to invoke** — manual invocation by the operator on any PR that adds or modifies a file matching the trigger surface (`*/HoneyDrunk.*.Database/`, `*/Backfill/`, or any model file referenced from a DbContext). At v1, manual only per ADR-0046 D3.
   - **## Context to load** — `adrs/ADR-0048-data-schema-evolution-and-migration-policy.md`, `adrs/ADR-0030-grid-wide-audit-substrate.md`, `adrs/ADR-0046-specialist-review-agents.md`, `constitution/invariants.md`, the target Node's `repos/HoneyDrunk.<Node>/integration-points.md` Schema Deployment Coordination section, and `catalogs/grid-health.json` for the target Node's `schema_evolution` field.
...
2. **Embed the seven-category rubric verbatim-in-substance from ADR-0048 D13:**
   ...
   - **D2 conformance — expand/contract pattern.** Is the change Expand, Migrate code, or Contract phase? Is it single-deploy destructive? If destructive, does the PR body declare `BreakingChange: true` with a reason? Is the tier appropriate (only Tier 2 internal stores may use a single-deploy destructive change)? Check against the forbidden-in-a-single-deploy list (column drops, renames, narrows, nullability flips, `NOT NULL` add without default, table drops, `NOT NULL` column add without default).
   - **D5 window adequacy — Expand/Contract phase pairing.** If the PR declares `SchemaPhase: Contract`, walk back to find the matching `SchemaPhase: Expand` PR by `SchemaChangeId`. Is the window between Expand and Contract within the store's tier window? Audit: indefinite (Contract never lands); Tier 0/1 customer-facing: 2 stable deploys or 14 days, whichever is longer; Tier 2 internal: 1 stable deploy; idempotency: 2 stable deploys or 30 days. Inadequate window is a blocking finding.
   - **D6 online primitives — index and constraint operations on large tables.** Identify the target tables. For tables ≥ 100k rows (read from the Node's `dr-runbook.md` per ADR-0036 D9 or query the staging snapshot), does the SQL project/publish script use Azure SQL online primitives (`CREATE INDEX ... WITH (ONLINE = ON)`, `WITH CHECK CHECK CONSTRAINT` after `WITH NOCHECK ADD CONSTRAINT`)? Missing online primitive on a large table is a blocking finding.
   ...
   - **D9 tenant scoping — multi-tenant table touched.** If the target table carries a `TenantId` column, is the schema deployment tenant-scoping-aware? Grid-wide tables (`IdempotencyKey`, `AuditEntry`, internal config) deploy once per environment without tenant logic. Tenant-scoped tables in a shared schema also deploy once; the DDL must use online primitives on any large-table schema deployment. Per-tenant schemas (not adopted by default) trigger a deeper review and the D9 second-branch follow-up ADR check.
   - **D10 rollback declaration — PR metadata present and adequate.** Every schema-changing PR must carry `RollbackStrategy: ForwardSchemaChange` (the default, with notes) or `RollbackStrategy: NonRollback` (with reason). Missing metadata is a blocking finding. For `NonRollback`, is the reason adequate?
   - **D12 tests present — DACPAC round-trip test added or updated.** Is there a corresponding test in `HoneyDrunk.<Node>.Tests.Integration.Containers/Database/<Node>DatabaseRoundTripTests.cs` (or the per-Node convention)? Does the test publish the DACPAC to a Testcontainers SQL Server database, assert the resulting schema against the EF model/SQL project contract, insert representative data, read it back, and assert integrity? For Expand-phase schema changes: does it also test old-shape data being read by new code (forward compatibility)? For Contract-phase schema changes: does it assert the old shape is gone and any old-shape data has been moved to the new shape? Missing or insufficient round-trip test is a blocking finding (also a CI gate failure per ADR-0047 D11 Tier 2b, but the `database` agent surfaces it during review for faster feedback).

   Plus the operational EF Core idiom check from D13:

   - **SQL project idiom — object model and publish artifact review.** Are schemas, tables, indexes, constraints, pre-deployment scripts, post-deployment scripts, and README notes split cleanly? Does the generated DACPAC publish script/report match the intended change and avoid hidden destructive drift?
...
- [ ] The agent's scope section names the trigger surface: `*/HoneyDrunk.*.Database/`, `*/Backfill/`, and any model file referenced from a `DbContext`
- [ ] The "When to invoke" section reflects manual invocation at v1 per ADR-0046 D3
- [ ] The Context to load section lists ADR-0048, ADR-0030, ADR-0046, `constitution/invariants.md`, the target Node's `integration-points.md` Schema Deployment Coordination section, and `catalogs/grid-health.json`
...
**ADR-0048 D13 — Specialist `database` agent.** Per ADR-0046, the Grid commits to specialist review agents for high-risk surface areas. Schema evolution is one of them. The agent walks every PR that touches `HoneyDrunk.<Node>.Database/`, `Backfill/`, or any file referenced from a `DbContext`. Checks: D2 conformance (single-deploy destructive change + `BreakingChange: true` PR metadata), D5 window adequacy (Expand/Contract phase pairing), D6 online primitives (row count vs 100k threshold), D8 Audit constraints (column drop / type narrowing / `NOT NULL` add forbidden on `AuditEntry`), D9 tenant scoping (multi-tenant table touched), D10 rollback declaration, D12 DACPAC round-trip test, SQL project idiom and publish artifact review. Required on every schema-touching PR. Invoked from `scope` agent when packets imply schema changes (packet 04 wires that).
...
**ADR-0046 D3 — Manual invocation only at v1.** No CI triggers; the operator decides when a lens applies. ADR-0048 D13 says the `database` agent is "required on every schema-touching PR" — at v1 the operator enforces that by invocation; CI-triggered specialist invocation is a deferred follow-up scoped under ADR-0046 D9.
...
**Context:**
- Goal: Give the Grid a specialist agent that enforces ADR-0048's schema-evolution policy on every schema-touching PR.
...
**Dependencies:**
- `work-item:00` — ADR-0048 must be Accepted so the agent's rubric can cite the three new schema-evolution invariants (93/94/95 or batch-shifted) as live rules.
