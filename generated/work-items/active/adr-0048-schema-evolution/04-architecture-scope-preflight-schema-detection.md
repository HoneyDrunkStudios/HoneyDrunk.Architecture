...
`.claude/agents/scope.md` is the scope-agent definition file (the agent producing this packet right now). It governs how scope-agent threads research, decompose, and author work items. Adding a pre-flight schema-change detector means: when the scope agent is given a feature request that might touch a `DbContext`, a `HoneyDrunk.<Node>.Database/` folder, or a relational data layout, the agent recognizes that the resulting packet will trigger `database` specialist review and acts accordingly — warning the operator, sequencing Expand and Contract phases correctly across packets, and including the right boilerplate (e.g. `[ExpandPhase("...")]`/`[ContractPhase("...")]` attribute usage, round-trip test acceptance criteria, `rollback declaration` attribute acceptance criteria) in the packet body.
...
- The request targets a Node whose `catalogs/grid-health.json` `schema_evolution` field is `sql-project-dacpac` and the work modifies the Node's `DbContext` or a model file referenced from it.
...
2. Adds boilerplate to the packet's Acceptance Criteria: `RollbackStrategy` PR-body metadata, `SchemaPhase`/`SchemaChangeId` if part of a paired Expand/Contract change, Tier 2b DACPAC round-trip test in `HoneyDrunk.<Node>.Tests.Integration.Containers/Database/<Node>DatabaseRoundTripTests.cs`, online-primitive usage if the target table is or might be ≥ 100k rows.
...
5. For document-store changes (`cosmos-schema-on-read`), follows ADR-0048 D7 instead of D2 — the change is a new field write or a `Backfill/Backfill-YYYYMMDD-{description}.md` runbook, not an EF migration; partition-key changes are flagged as a deferred-ADR follow-up (D7 names it as the most expensive document-store evolution shape, gated on a follow-up ADR).
...
- The per-Node `repos/HoneyDrunk.<Node>/integration-points.md` Schema Deployment Coordination section — gives the scope agent the per-Node schema deployment framework and cross-Node coordination context.
...
- The per-Node `integration-points.md` Schema Deployment Coordination section
...
## Acceptance Criteria
- [ ] `.claude/agents/scope.md` carries a pre-flight schema-change-detection sub-section with the trigger conditions named explicitly (entity/column/table/index DDL verbs; `sql-project-dacpac` or `cosmos-schema-on-read` Node; `DbContext` modification)
...
- [ ] `.claude/agents/scope.md` context-loading list includes ADR-0048, the per-Node `integration-points.md` Schema Deployment Coordination section, and `catalogs/grid-health.json`'s `schema_evolution` field
...
**ADR-0048 D13 — Specialist `database` agent on every schema-touching PR; also invoked from `scope` when packets imply schema changes.** "The agent is also invoked from the `scope` agent when packets imply schema changes — the packet pre-flight check warns the operator that the work will trigger a migration review, so the packet author can sequence Expand and Contract correctly from the start."
...
**ADR-0048 D7 — Document stores follow schema-on-read.** No DDL; new fields by writing; backfill via `Backfill/Backfill-YYYYMMDD-{description}.md` runbook; partition-key change is a container replacement and a deferred-ADR trigger. The detector switches to the D7 path when the target Node's `schema_evolution` is `cosmos-schema-on-read`.
...
**ADR-0048 D10 — `rollback declaration` attribute.** Every schema-changing PR carries `[Rollback(Strategy = ..., Notes/Reason = "...")]`. The detector includes this in the packet's acceptance criteria.

**ADR-0048 D12 — DACPAC round-trip test in `HoneyDrunk.<Node>.Tests.Integration.Containers/Database/<Node>DatabaseRoundTripTests.cs`.** Tier 2b CI gate per ADR-0047 D11. The detector includes this in the packet's acceptance criteria.
...
- **Document-store path uses D7, not D2.** When the target Node's `schema_evolution` is `cosmos-schema-on-read`, the detector switches paths; no EF migration is authored, and a `Backfill/Backfill-YYYYMMDD-{description}.md` runbook is the artifact.
