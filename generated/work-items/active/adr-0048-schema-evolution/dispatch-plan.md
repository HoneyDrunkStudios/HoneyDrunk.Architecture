...
- **Packet 09** was originally filed as a Standards rollback-attribute packet. Rollback posture now lives in PR-body `RollbackStrategy` metadata and SQL project README notes, not a shared runtime attribute. The packet 09 filename is retained as historical filed scope even though its execution semantics are superseded.

Filed GitHub issues and project-state updates for those historical packets are reconciled by `generated/work-items/proposed/2026-06-14-architecture-dacpac-filed-packet-reconciliation.md`. That proposed reconciliation packet is the executor-safe supersession path: before packet 05, packet 07, packet 09, or any ADR-0072 filed issue that still instructs EF migration production deployment can proceed, human triage must promote/file the reconciliation packet or the specific owning issue must have a supersession comment or project-state note pointing the executor back to this dispatch-plan section and ADR-0048 D1/D3/D11.

Supersession comments were verified through the GitHub API on 2026-06-15 so direct issue readers see the DACPAC pivot before execution:
