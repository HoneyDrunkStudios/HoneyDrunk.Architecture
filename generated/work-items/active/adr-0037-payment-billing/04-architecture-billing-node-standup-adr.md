...
**Dependencies:**
- `work-item:00` — soft. ADR-0037 should be Accepted so the new standup ADR references it as a live decision.
- `work-item:01` — soft. The `HoneyDrunk.Billing` planned-Node catalog/context registration should exist so the standup ADR references a registered Node.
