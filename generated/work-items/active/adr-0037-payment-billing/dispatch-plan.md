...
BDR-0001 is **not** in any `dependencies:` array — it is a business decision record with its own action-item track, not a packet, and it has no `filed-work-items.json` entry to resolve against. Packet 02's hard dependency on the BDR-0001 address being final is enforced by packet 02's Human Prerequisites checklist and the CAUTION block at the top of that packet.
