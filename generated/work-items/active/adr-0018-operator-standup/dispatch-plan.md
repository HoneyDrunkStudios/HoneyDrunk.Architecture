...
**Packet 02 cannot be filed until packet 01 has been filed.** Packet 02's `dependencies: ["work-item:01"]` makes this explicit at the data layer, and per invariant 24 packet 02 becomes immutable the moment it lands as a GitHub Issue — so the dependency resolves to a `work-item:01` issue number that must already exist. The filing pipeline (`file-work-items.yml`) processes packets in two-digit-prefix order on each push, so as long as packets 01 and 02 land in the same push the wiring is automatic.
...
The `file-work-items.yml` workflow auto-files on push to `generated/work-items/active/**/*.md`. No manual `gh issue create` commands. Verify by checking The Hive (org Project #4) for the new items and their blocking edges.
