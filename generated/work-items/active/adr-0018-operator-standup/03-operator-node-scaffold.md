...
> **Invariant 26:** Work items for .NET code work must include an explicit `## NuGet Dependencies` section. `HoneyDrunk.Standards` must be on every new .NET project.
...
## Dependencies
- `work-item:01` — catalog registration must land first.
- `work-item:02` — four invariants must exist before this packet's acceptance criteria reference them.
- `work-item:02b` — repo settings verified, local clone confirmed.
