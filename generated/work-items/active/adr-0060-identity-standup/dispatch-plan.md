...
2. **Constitution invariants** — three new invariants from D1/D6/D7 added to `constitution/invariants.md` at numbers **64, 65, 66** (next three reserved slots above the current reservation framework default). New `## Identity Invariants` section. 64 = user-record ownership; 65 = internal-token issuance exclusivity; 66 = Identity contract-shape canary. ADR-0060 Consequences `### Invariants` subsection finalized with the assigned numbers. `repos/HoneyDrunk.Identity/invariants.md` placeholders substituted. Packet 04 source-file placeholders substituted pre-push (invariant 24's pre-filing carve-out).
...
| 02 | [Add ADR-0060's three new invariants to the Grid constitution (default 64-66)](./02-architecture-identity-invariants.md) | Architecture | 1 | Agent | 01 |
...
- **SonarCloud onboarding for HoneyDrunk.Identity.** Follows the pattern from `generated/work-items/active/adr-0011-code-review-pipeline/06-kernel-sonarcloud-onboarding.md`. Separate follow-up packet, post-`v0.1.0`. Flagged in packet 04's Human Prerequisites.
...
The default assignment for this initiative's three constitutional invariants is **64, 65, 66** (next three reserved slots above the current reservation framework default — set by ADR-0044's invariants 52 and 53). Packet 02 of this initiative runs a `rg -n '^[0-9]+\.' constitution/invariants.md | tail -n 20` collision check at edit time and assigns whatever the actual next three reserved slots are.

**Collision pre-claims to be aware of (do not block on, but check):**

- **In-flight AI-sector standups** (ADR-0020 Agents, ADR-0021 Knowledge, ADR-0022 Memory, ADR-0023 Evals, ADR-0024 Flow, ADR-0025 Sim) — each carries its own invariant-numbering packet that may land between this scoping and packet 02's edit time. If any AI-sector packet lands first and claims 64-66 (or part of it), this initiative's three entries shift up together.
