...
> **Invariant 26 — Work items for .NET code work include a `## NuGet Dependencies` section.**
...
**Dependencies:**
- `work-item:03` — hard. The PagerDuty integration (Events API v2 key, dedicated probe service) must exist before the probe can fire it.
- `work-item:04` — hard. The probe consumes `IIncidentPagingSender` from packet 04; the contract must exist.
