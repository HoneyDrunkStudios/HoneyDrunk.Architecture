...
## Dependencies
- `work-item:01` — Architecture catalog registration must have merged so the catalogs already point at the eventual repo and the context folder exists. The GitHub-side actions are technically independent of packet 01's merge, but the scaffold (packet 03) is not; the dependency keeps the chain visible on The Hive.
...
- Invariant 24 (work items are immutable once filed; pre-filing amendments permitted) — the repo must exist before packet 03 can be filed against it.
