...
dependencies: ["work-item:01", "work-item:01a"]
...
## Dependencies
- `work-item:01` — **hard**. Agent definition + capability-matrix row + report directory must exist so the walkthrough can cross-link them.
- `work-item:01a` — **hard**. The App + Vault + secrets + RBAC + rotation disposition must exist and the hand-off note must be available before the walkthrough can record concrete values.
...
- **PR metadata:** `Authorship: agent-claude-code` + `Work Item: HoneyDrunkStudios/HoneyDrunk.Architecture#<issue-number>` once filed.
...
- PR metadata: `Authorship: agent-claude-code` + `Work Item: <path>`.
