...
**Packet 04 cannot be filed before packet 03 is closed** — `file-work-items.sh` cannot create an issue on a repo that does not exist.
...
`file-work-items.yml` auto-files. Packet 04 filing is gated on packet 03's repo creation (handled by the Next-Steps script inside packet 03).
