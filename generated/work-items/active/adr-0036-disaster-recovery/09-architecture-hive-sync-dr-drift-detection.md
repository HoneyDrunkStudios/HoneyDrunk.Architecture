...
**Dependencies:**
- `work-item:01` — hard. The `dr_tier` field must exist in `grid-health.json` for the completeness check to read.
- `work-item:02` — hard. `generated/restore-drills/` and its log schema must exist for the currency check to read.
