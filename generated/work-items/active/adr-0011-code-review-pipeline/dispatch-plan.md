...
- [ ] `HoneyDrunk.Actions`: Inject packet link into PR body via `agent-run.yml` `work-item-path` input + post-hoc assert step — [`03-actions-agent-run-packet-link.md`](03-actions-agent-run-packet-link.md)
...
- `agent-run.yml` accepts `work-item-path` and (a) injects `> Work Item: <permalink>` markdown into the agent's prompt envelope when supplied AND (b) runs a post-hoc "Assert PR-body packet link" step that mechanically asserts the line into any PR the agent opened. Existing callers unaffected.
...
```bash
PACKETS="generated/work-items/active/adr-0011-code-review-pipeline"
...
- **No `file-work-items` driver currently consumes `agent-run.yml`'s new `work-item-path` input.** Packet 03 ships the input + the post-hoc assert step; an existing or future driver workflow (out of scope for this initiative) will populate it. Until then, the input is optional and unused — which is the correct pre-rollout state per packet 03's design.
