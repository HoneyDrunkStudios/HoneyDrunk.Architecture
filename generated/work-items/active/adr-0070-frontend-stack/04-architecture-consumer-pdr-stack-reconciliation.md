...
Cons: leaves the policy unstable. A third PDR that wants native gets to claim the same carve-out by precedent; the policy degrades from "no native" to "ad hoc native" without explicit policy commitment. Anti-pattern under invariant 24's spirit (work items are immutable for the same reason: ad-hoc divergence corrodes the substrate).
...
**Dependencies:**
- `work-item:00` — hard. ADR-0070 must be Accepted before its conflict with PDR-0005/0008 is binding policy.
