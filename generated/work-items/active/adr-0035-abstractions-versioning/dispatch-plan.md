...
**Fan-out work-item:** packet 04 is a per-Node fan-out — the `file-work-items` agent expands it into one issue per repo across the 12 repos in `target_repos` (Kernel, Transport, Vault, Vault.Rotation, Auth, Web.Rest, Data, Audit, Pulse, Notify, Communications, Observe), or files it as a tracking issue with 12 sub-tasks. If at filing time `HoneyDrunk.Audit` is not yet package-publishing, it is dropped from the fan-out and noted.
...
The acceptance co-landing with ADR-0034 packet 00 is **not** in any `dependencies:` array — it cannot be (cross-initiative `work-item:NN` does not resolve, and ADR-0034's packet 00 is not yet a filed issue with a `{Repo}#N` id at scope time). It is a merge-time human coordination, called out in packet 00's body and here.
