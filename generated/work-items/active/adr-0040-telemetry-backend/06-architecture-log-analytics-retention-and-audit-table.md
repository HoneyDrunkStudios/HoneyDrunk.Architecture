...
**Dependencies:**
- `work-item:02` — hard. The `dev` App Insights resource and its Log Analytics workspace must exist before per-table retention can be configured.
- **work-item:05** — soft coordination (not a hard `dependencies:` edge — different repo, both can proceed). The Audit and `evals.sensitive` table names must agree between packet 05's exporter routing and this packet's table creation.
