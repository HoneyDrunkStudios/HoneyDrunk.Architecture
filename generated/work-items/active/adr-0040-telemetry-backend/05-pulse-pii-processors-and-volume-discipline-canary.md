...
- **Packet 04 lands first.** Packet 04 also touches `Sink.AzureMonitor`; this packet depends on `work-item:04` and rebases on it.
...
**Dependencies:**
- `work-item:03` — hard. The processors compose into the `TracerProvider`/`LoggerProvider` builder seams `HoneyDrunk.Telemetry.Sink.AzureMonitor` exposes.
- `work-item:04` — hard. Packet 04 also modifies `Sink.AzureMonitor`; this packet is sequenced after it to avoid a merge conflict on the shared project.
