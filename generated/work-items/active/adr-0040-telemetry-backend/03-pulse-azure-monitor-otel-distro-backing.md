...
> **Invariant 26 — Work items for .NET code work must include a `## NuGet Dependencies` section, and `HoneyDrunk.Standards` is on every .NET project** (analyzers, `PrivateAssets: all`).
...
**Dependencies:**
- `work-item:00` — soft. ADR-0040 should be Accepted before its core implementation lands.
- `work-item:02` — hard for the live smoke check (the `dev` App Insights resource + Vault connection string). The code can be authored and unit-tested without it; the end-to-end verification needs it.
