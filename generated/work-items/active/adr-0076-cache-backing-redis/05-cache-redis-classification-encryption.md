...
- `work-item:03` — the bare `HoneyDrunk.Cache.Redis` package must exist before the encrypted sibling can be added. (Packet 04 — telemetry/health/error-reporter — is NOT a dependency; packets 04 and 05 are siblings against the in-progress `[0.1.0]`.)
...
**Dependencies:** `work-item:03` only. (Packets 04 and 05 are sibling packets against the same in-progress `[0.1.0]` version; whichever merges second rebases.)
