...
1. **ADR-0027 (Notify Cloud standup) is Proposed**, not Accepted. The Notify Cloud repo does not exist on disk; `catalogs/nodes.json` has no `honeydrunk-notify-cloud` entry; `file-work-items.yml` cannot file an issue against a repo that does not exist.
...
## Constraints
- **No filing against `HoneyDrunk.Notify.Cloud`.** The repo does not exist on disk and is not yet a target for `file-work-items.yml`. This packet is purely an Architecture-repo specification document.
...
**Dependencies:**
- `work-item:00` — ADR-0067 should be Accepted before its production-implementation specification is authored as canonical.
