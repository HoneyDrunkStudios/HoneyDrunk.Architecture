...
## Summary
Confirm and document the canonical shape of the two audit event-types ADR-0067 D10 commits — `RateLimitRejected` (every hard 429) and `QuotaOverageBilled` (Pro/Scale tenants over their monthly quota; request proceeded, billing meter incremented) — in `HoneyDrunk.Audit`'s docs. This is a docs-only work-item: no contract change, no code change. The two event-types ride the existing `AuditEntry.EventName` field; this packet pins the canonical names, categories, outcomes, and metadata-field shapes so Notify Cloud (when its standup completes) emits the right entries and downstream forensic queries find them by stable name.
...
**Dependencies:**
- `work-item:00` — ADR-0067 should be Accepted before its event-types are documented as canonical.
