...
|---|------|------|--------|-------|---------|---------------|------------|
| 1 | Harden ADR-0044 Phase 1 transport + prepare Phase 2 | packet | In progress | **Phase 1→2 gate** | Phase 1 is live in Architecture through artifact + OpenClaw cron/poll, and Actions-side authorship/size/label foundations have landed. Before broad fan-out, finish the remaining transport and label-fanout verification gaps. | Webhook delivery provisioned/verified or cron fallback explicitly retained; stale/superseded ADR-0044 issues reconciled; Actions#86 label fanout run completed/verified or explicitly deferred before Node rollout | None |
