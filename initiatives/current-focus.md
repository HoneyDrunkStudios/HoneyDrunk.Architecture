...
| 4 | Deploy Notify + Pulse to dev | initiative (ADR-0015) | In progress (2/5) | In-flight operational work; dev-environment bring-up. Was the prior primary focus and still real. | `Notify#3`, `Notify#4`, `Pulse#3` closed; all three services running on `cae-hd-dev` | ADR-0033 environment-gated trigger packets (`Notify#19`, `Notify#20`, `Pulse#18`) must land before dev deploy verification resumes; Pulse image also gated on upstream Ubuntu `sed` CVE-2026-5958 |
