...
|---|------|------|--------|-------|---------|---------------|------------|
| 1 | Land ADR-0044 — Cloud Code Review (Phase 1) | adr-acceptance + packet | Proposed | **Phase 1 of 4** | Substrate. The review gate catches defects in everything downstream; landing before the AI-sector standup wave produces real PR volume is the difference between catching defects at PR time vs incident time. | ADR-0044 Accepted; Phase 1 MVP (`job-review-request.yml` + Grid Review Runner) enabled on `HoneyDrunk.Architecture` only and running on every non-draft PR | None |
...
| 7 | ADR-0044 D3 rubric rollout | packet | Not started | n/a | The 20-category rubric must land in `review.md` and upstream agent files for #1 and #6 to function as designed. | `.claude/agents/review.md` carries the 20-category rubric; `scope` / `adr-composer` / `pdr-composer` / `refine` / `node-audit` reference ADR-0044 D3 | #1 (Land ADR-0044) |
