...
- [ ] **ADR-0077 Infrastructure-as-Code (Bicep) consolidation** — accept the amended ADR (invariants 90–92) and stand up the new `HoneyDrunk.Infrastructure` repo (`modules/` + `platform/` + `nodes/`); module registry dropped, deploy/lint pipeline stays in Actions per ADR-0012. Forcing function: the ADR-0015 dev deploy proved the hand-provisioning pain (apps built live via `az rest`). Runs in parallel with HoneyHub; Notify Cloud's cloud bring-up composes on the reproducible provisioning this delivers. **As of 2026-06-09: ADR-0077 is Accepted (amended) and per operator the Bicep IaC work is fully done and tested — not "pending verification." The only residual is catalog bookkeeping: `HoneyDrunk.Infrastructure` Node registration (#571/#572) + repo scaffold (packet 11) still unfiled — folded into the reconcile sweep, no longer a standalone current-focus row.** *(current focus #9 — reconcile sweep — ADR-0077 catalog residual)*
