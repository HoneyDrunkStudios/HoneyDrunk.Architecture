...
17. Implementation-notes completion-gate holds (invariant 110): initiatives whose tracked issues are all closed but which lack a conforming implementation-notes record — `implementation-notes.md` in the initiative's packet folder, plus (for decision-driven initiatives) a `## Implementation Notes (YYYY-MM-DD)` pointer section in the governing ADR/PDR/BDR. Step 3 holds these `In Progress` rather than completing/archiving them; surface each here so the implementing agent authors the missing record. `hive-sync` never authors it.
