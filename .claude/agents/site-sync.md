...
name: site-sync
description: >-
  Synchronize the HoneyDrunk Studios website with architecture changes. Use when a Node is added/renamed/removed, a version is bumped, a signal changes, an ADR is accepted, relationships change, sectors are restructured, or any catalog data diverges from the website's data/schema files.
tools:
  - Read
  - Grep
  - Glob
