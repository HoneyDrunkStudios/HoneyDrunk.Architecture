...
| GitHub Actions (CI/CD) | [ADR-0012](../../adrs/ADR-0012-grid-cicd-control-plane.md) | `HoneyDrunk.Actions` is the canonical CI/CD control plane per [ADR-0012](../../adrs/ADR-0012-grid-cicd-control-plane.md). Every Node consumes its reusable workflows (`pr-core.yml`, the `job-*` library); per-Node `.github/workflows/*.yml` calls them. The Grid CI/CD invariants (37–41 per [ADR-0012](../../adrs/ADR-0012-grid-cicd-control-plane.md)) reference Actions semantics directly. | Months. Re-author every reusable workflow in the target CI platform's primitives (GitLab CI, Buildkite, etc.); rewire OIDC federation for Azure deploy per [ADR-0005](../../adrs/ADR-0005-configuration-and-secrets-strategy.md). |
...
| PR workflow + Grid review runner | [ADR-0044](../../adrs/ADR-0044-grid-aware-cloud-code-review-and-ai-authored-pr-discipline.md), [ADR-0079](../../adrs/ADR-0079-multi-perspective-pr-review-stack.md), [ADR-0086](../../adrs/ADR-0086-pull-based-local-worker-grid-review-runner.md) | Per ADR-0086, `job-review-request.yml` normalizes managed PR labels and enqueues a review via a GitHub-native **label + structured-comment queue**; the home-server **local worker** polls that queue, claims one PR/head-SHA at a time, and runs the canonical `.claude/agents/review.md` under the Codex CLI and Claude Code CLI; review comments post via the GitHub API. The `.honeydrunk-review.yaml` per-repo opt-in lives in the repo per [invariant 52](../../constitution/invariants.md). | Weeks. Re-author the label/comment-queue trigger against the target's PR-label + PR-comment API; the agent definition and local worker are SCM-independent. |
| Local review/agent runner (ADR-0086) | [ADR-0086](../../adrs/ADR-0086-pull-based-local-worker-grid-review-runner.md) — supersedes the ADR-0044 D1/D5 OpenClaw signed-webhook transport | The home-server local worker polls the GitHub label/comment queue and runs review + scheduled agent jobs under local CLIs against subscription auth. The integration shape is GitHub-native (PR labels, PR-comment API, repo-permission model). _OpenClaw's signed-webhook review transport is **superseded by ADR-0086** and is no longer the review path._ | Weeks. Re-author the queue poller against the target's PR-label/PR-comment API; the agent definitions are preserved. |
...
3. **Solo-dev + AI-agent SDLC is the operator's posture.** The ADR-0086 local-worker review runner and its GitHub label/comment queue, the `file-work-items.yml` automation, the `hive-sync` reconciliation — these are all built against GitHub's specific shape. Re-architecting them against a portability layer would erode the AI-multiplier discipline the charter explicitly underwrites.
...
- PR-label + PR-comment API (and a structured-comment queue) equivalence sufficient for the ADR-0086 local-worker review runner.
...
Per [ADR-0080](../../adrs/ADR-0080-vendor-lock-in-posture-and-exit-readiness-hedges.md) D8, the concrete per-surface migration mechanics — the specific steps for moving source, Actions, Projects, the ADR-0086 review runner, and per-repo CI to a named replacement — are deferred until a real trigger fires and the re-evaluation conversation produces "Exit." This stub commits the structure, the SCM-migration pre-condition, and the honest cost range; the per-surface mechanics are filled in at the migration's own initiative.
...
- [ADR-0086](../../adrs/ADR-0086-pull-based-local-worker-grid-review-runner.md) — pull-based local worker as the Grid review runner (supersedes the OpenClaw review transport)
