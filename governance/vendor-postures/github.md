...
3. **Solo-dev + AI-agent SDLC is the operator's posture.** The ADR-0086 local-worker review runner and its GitHub label/comment queue, the `file-packets.yml` automation, the `hive-sync` reconciliation — these are all built against GitHub's specific shape. Re-architecting them against a portability layer would erode the AI-multiplier discipline the charter explicitly underwrites.
