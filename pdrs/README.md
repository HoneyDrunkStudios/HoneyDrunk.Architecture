...
| [PDR-0011](PDR-0011-honeyhub-v1-agent-cockpit-and-usage-governance.md) | HoneyHub v1: Agent Cockpit and Usage Governance | Accepted | 2026-06-06 | Meta / AI / Platform | Names HoneyHub **v1 = the Agent Cockpit** (govern/monitor local Codex/Claude Code/Copilot sessions — transcripts, token/model governance, mobile monitoring) as the commercial wedge under PDR-0001, testable on the operator's day-job agent usage ahead of Notify Cloud readiness. **Re-sequences PDR-0009** (internal Grid read-layer / daily-driver workspace) to a *later* HoneyHub layer — not superseded; PDR-0009's PRs-as-artifacts boundary is inherited. Web/PWA + secure local runner bridge → local agent tools/git/gh on the ADR-0086 local-worker substrate; one-session-one-backend, chat-shaped dispatch, estimated-first usage governance (advisory in local v1), local-first with cloud/API as a later provider over the same contract. **Implemented by the HoneyHub Local Runner Bridge ADR** (promote ADR-DRAFT). Re-sequenced **ahead of Notify Cloud (PDR-0002)** for near-term focus, with hard kill criteria. |
