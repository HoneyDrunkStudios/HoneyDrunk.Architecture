...
| [PDR-0002](PDR-0002-notify-as-a-service-first-commercial-product.md) | NovOutbox: First Commercial Product on the Grid | Proposed | 2026-05-02 | Ops / Meta | Names NovOutbox as the Grid's first commercial app, superseding the earlier HoneyDrunk Notify / Notify Cloud customer-facing names. Reframes PDR-0001 §C — HoneyHub is no longer the external-facing product. Open core: Notify engine OSS (FSL/BSL), proposed `HoneyDrunk.Notify.Cloud` wrapper private unless the technical name changes before creation. |
...
| [PDR-0010](PDR-0010-agent-action-ledger-hosted-forensic-record-for-ai-agents.md) | Agent Action Ledger: Hosted Forensic Record for AI-Agent Actions | Proposed / Exploring | 2026-05-27 | Core / AI · Ops · Meta | Records the substrate-cluster bet identified by the product-strategist's Scout pass as the top 2-to-3-year SaaS opportunity behind NovOutbox. Captures thinking only — not a green light. Hard sequencing: NovOutbox (PDR-0002) launch -> Audit Phase-1 (ADR-0031) lands -> Audit Node v1.0 -> strategist Critic-mode pass -> only then considered for promotion. Default: hosted wrapper over existing Audit + IModelRouter + Capabilities + Vault, no new Grid Node. Cost-as-hook, audit-as-product boundary. Hard kill criteria (NovOutbox sunset -> collapse; Audit Phase-1 slips past 9 months -> collapse; no >$30/mo buyer in 6 months post-launch -> kill; no stuck design partner past month 3 -> kill). |
| [PDR-0011](PDR-0011-honeyhub-v1-agent-cockpit-and-usage-governance.md) | HoneyHub v1: Agent Cockpit and Usage Governance | Accepted (amended 2026-06-06) | 2026-06-06 | Meta / AI / Platform | Names HoneyHub **v1 = the FREE, local Agent Cockpit** (govern/monitor local Codex/Claude Code/Copilot sessions — transcripts, token/model governance, mobile monitoring) as the **lead near-term build focus among parallel portfolio threads** (charter §portfolio) and the operator's dogfood wedge. **Re-sequences PDR-0009** (internal Grid read-layer / daily-driver workspace) to a *later* HoneyHub layer — not superseded; PDR-0009's PRs-as-artifacts boundary is inherited. Mobile-PWA + desktop in parallel (one shared web UI; Tauri-class shell bundles the bridge); three backends via official CLIs (Codex/Claude Code full, Copilot CLI best-effort); one chat surface, parallel sessions, agent-discovery; one-session-one-backend; estimated-first usage governance (advisory in local v1); local-first. **Amended 2026-06-06** (two product-strategist Critic MODIFY passes): corrects the overclaimed commercial framing — **NovOutbox (PDR-0002) keeps its Q3 commercial-trial slot, not demoted**; routing is a **free-tier** "capability/cost-fit + optimize-your-own-subscriptions" feature (never cap-dodging); the one gated commercial candidate is **BYOK cloud execution** (validation-probe-gated), the **Dev surface is internal-default**, coaching is a retention feature. Adds a **`[Firm]` cloud=BYOK-only / never-touch-subscription-auth boundary** (separates HoneyHub from the ADR-0088/OpenClaw banned shape) + **Legal-ToS** and **Differentiation** kill criteria + a Decision Ledger (`[Firm]`/`[Provisional]`) flexibility posture. **Implemented by the HoneyHub Local Runner Bridge ADR** (promote ADR-DRAFT) on the ADR-0086 local-worker substrate. |
