...
The canonical procedure every Grid Node standup follows. Before this document existed, every standup (Communications, the nine AI-sector Nodes, Audit, NovOutbox, the Cache/Identity/Files pre-commits) re-derived its own checklist from the most recent precedent, and the drift was discovered the next time a Node was stood up. This document is the single source future standup packets, ADRs, and AI agents reference instead of re-deriving.
...
This procedure (and invariant 102) applies only to artifacts that are **Nodes** under the D2 node-class taxonomy below. Operator-internal automation infrastructure — Azure Key Vaults provisioned for agents (e.g. `kv-hd-docs-sync-prod` per ADR-0085), GitHub Apps registered for cross-repo automation, the ADR-0086 local worker, retired OpenClaw schedules, the home server itself (ADR-0081 superseded by ADR-0088; hardware premise re-homed under ADR-0086), credential inventories (ADR-0083's `infrastructure/reference/sensitive-inventory.md`) — is **NOT** a Node. Invariant 102's ten-item registration checklist does not bind these artifacts: they have no `catalogs/nodes.json` row (they are not Nodes), no `repos/{Node}/` context folder, no `.honeydrunk-review.yaml`, no `pr.yml` calling `pr-core.yml`, and no per-class org-secret matrix. Their governance lives in their owning ADR (ADR-0083, ADR-0085, ADR-0086, ADR-0088, ...) and the walkthroughs that owning ADR commits.
...
| Ops Deployable .NET | `ops-deployable-dotnet` | One or more `.Abstractions` packages plus one or more deployable Services (Container Apps / Functions). | Notify, Pulse.Collector, NovOutbox, Operator, Communications (when deployed) |
...
- Branch protection applied on `main` — PR required before merge, `pr-core / core` required status check (Invariant 31: *every PR traverses the tier-1 gate before merge* — build, unit tests, analyzers, vuln scan, secret scan are required branch-protection checks delivered via `pr-core.yml`; invariant 31 itself is scoped to .NET repos, and non-.NET D2 classes require their class-specific check instead — e.g. `pr / build` for `studios-typescript-native`, per invariant 102 clauses 8/9), no force-push, no deletion.
...
4. **Catalog entries** in `catalogs/nodes.json` (Node row with all fields — id, name, sector, signal, cluster, energy, priority, flow, tags, repo link, optional `visibility` when the repo is not public, `long_description`, `foundational`, `strategy_base`, `tier`, `time_pressure`, `done`, `cooldown_days`), `catalogs/relationships.json` (every consumes/consumed_by edge ADR-pinned, no cycles per Invariant 4: *the dependency graph is a DAG; Kernel is always at the root*), and `catalogs/grid-health.json` (contract surface, canary expectation, DR tier per ADR-0036). Hive-sync owns the live state per Phase D. *[Phase A]*
5. **Repo created** at the correct visibility — public by default per the standing repo-visibility rule; private only with an ADR-recorded revenue/compliance/experiment carve-out (precedent: ADR-0027 D2 for NovOutbox, the first private repo on the Grid). *[Phase B]*
6. **Branch protection on `main`** — PR required, `pr-core / core` required check (Invariant 31, restated at step under Phase B; non-.NET D2 classes require their class-specific check instead — e.g. `pr / build` for `studios-typescript-native`, per invariant 102 clauses 8/9), no force-push, no deletion, signed commits not required (matches existing Grid posture). Additional required checks (`api-compatibility / abstractions-shape`, `job-sonarcloud / sonarcloud` for public repos) are added in a *follow-up* branch-protection update after the throwaway breaking-change PR confirms the canary fires post-merge — the same pattern as the Audit and AI standups. *[Phase B; follow-up update post-scaffold]*
...
10. **CodeRabbit integration** per ADR-0079 D1 — covered at the org level, **no per-repo action**. CodeRabbit's GitHub App is installed org-wide ("All repositories") and the canonical Grid config is enforced via **Global Overrides** (the operator one-time setup in `infrastructure/walkthroughs/coderabbit-org-setup.md`, applying `templates/.coderabbit.yaml`). A new repo is therefore reviewed by CodeRabbit the moment it exists — the standup **verifies** coverage (a non-draft PR draws a CodeRabbit summary comment) but commits nothing. A per-repo `.coderabbit.yaml` is **optional**, added only to refine the org baseline for repo-specific patterns (e.g. Vault/Audit path instructions per `templates/README.md`); it is never required for coverage. *[Phase C — verify only]*
...
15. **`.github/copilot-instructions.md`** — a per-repo file pointing back to Architecture's copilot-instructions plus any repo-specific addenda. *[Phase C]*
