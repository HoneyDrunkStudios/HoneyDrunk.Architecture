...
103. **Every credential, identifier, secret, or load-bearing identity binding the Grid holds must have a row in `infrastructure/reference/sensitive-inventory.md`; rotation-needing rows additionally carry a `Current Expiration`, a walkthrough, and an open standing rotation issue.**
    ...
    **Planned / pre-provisioning carve-out (per ADR-0083 D6).** A `Rotates: yes` row whose artifact is **not yet provisioned** (`status: planned` in `Notes`, e.g. a declared-but-not-live API key) or whose per-provider walkthrough is a **named pending follow-up** (the `Rotation Procedure` cell points at a not-yet-authored walkthrough, e.g. a webhook-bridge secret awaiting its rotation flow) requires **only** the inventory row until the artifact goes live. Per ADR-0083 D6's onboarding hook, the `Current Expiration`, the walkthrough, and the standing issue land **when the artifact is provisioned / its walkthrough is authored** — they are not required for the pre-live row. Such rows carry a far-future sentinel `Current Expiration` (e.g. `2099-01-01`) that `external-credentials-check.yml` treats as no-escalation, so the drift workflow neither blocks nor pages on them.
