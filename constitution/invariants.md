...
12. **Semantic versioning with CHANGELOG and README.**
    ...
    - **Per-package `CHANGELOG.md`** (inside each package directory): Carries detailed entries only when that specific package has functional changes — do not fabricate change bullets for packages version-bumped solely to align with the solution. However, because `HoneyDrunk.Actions`' NuGet Version Consistency Check requires a `## [<version>]` heading in *every* inline-versioned package's CHANGELOG (per-package parity + the release-notes generator depend on it), each alignment-only package still gets that heading plus a single `version alignment with the solution <version> release; no behavior change` line. That minimal entry is the **gate-required form, not prohibited noise** (see invariant 27). Consumers use these to understand what changed in the package they depend on.
...
27. **All projects in a solution share one version and move together.**
    When a version bump is warranted, every `.csproj` in the solution (excluding test projects) is updated to the same new version in a single commit. Partial bumps — where some projects in a solution are on a different version than others — are forbidden. Releases are triggered by pushing a git tag; agents never push tags. The first packet to land on a solution in an initiative bumps the version; subsequent packets on the same solution append to the CHANGELOG only. The repo-level `CHANGELOG.md` must always get an entry for the new version. Per-package changelogs carry detailed entries only for packages with actual changes — do not fabricate change bullets for alignment-only bumps (see invariant 12). Because `HoneyDrunk.Actions`' NuGet Version Consistency Check requires a `## [<version>]` heading in *every* inline-versioned package's CHANGELOG, each alignment-only package still gets that heading plus a one-line `version alignment with the solution <version> release; no behavior change` note — that minimal entry is the gate-required form, not the prohibited noise, and removing it would fail the gate. See invariant 26 and ADR-0008.
