...
|-------|---------|----------|----------|-------------|
| **scope** | New work to decompose | Goal/user request, ADRs, catalogs, repo boundaries | Work items, dispatch plan, handoff | Execute work, write code, open PRs |
...
| **refine** | Draft exists, needs iteration | Draft doc (ADR, PDR, work item, design), feedback | Revised draft | Author from scratch, make binding decisions |
...
> **Status:** `hive-sync` is rolling out across ADR-0014 work items 01-06. The work-item lifecycle (active → completed), `board-items.md`, `initiatives/proposed-adrs.md`, ADR/PDR auto-acceptance, README index sync, and `initiatives/drift-report.md` surfaces become live as Work Items 02-06 land.
...
existence — the same discipline `scope` follows for work items (`proposed/` → `active/`).
...
Do I need to decompose a goal into executable work items?
  → yes → scope

Does the work item already need to become a GitHub Issue?
  → yes → file-issues
...
Do Architecture-repo files need reconciliation with The Hive (initiatives, work-item lifecycle, non-initiative items, Proposed-ADR queue)?
...
- **Agents do not modify files inside other repos directly.** Cross-repo work goes through work items → GitHub Issues → Codex.
...
| file-issues | `generated/work-items/active/` dispatch plan, `copilot/issue-authoring-rules.md` |
| hive-sync | `generated/work-items/filed-work-items.json`, `catalogs/grid-health.json`, `catalogs/nodes.json`, `initiatives/`, `adrs/ADR-*.md` (frontmatter), GraphQL Hive board query results |
...
| review | ADRs referenced in work-item frontmatter, `constitution/invariants.md` |
...
Goal / User Request
  ...
  │
  └─ scope       → generated/work-items/active/{initiative}/
                      ├─ dispatch-plan.md
                      ├─ handoff-{wave}.md
